...
export default class App extends Component {
  constructor(props) {
    console.warn('process.pid', process.pid);
    super(props);
    if (!APP_ID) {
  ...
  getRtcEngine() {
    if (!this.rtcEngine) {
      ...
      this.rtcEngine.initialize(APP_ID, 0xffffffff, {
  ...
  subscribeEvents = rtcEngine => {
    ...
    rtcEngine.on(
      ...
      (speakers, speakerNumber, totalVolume) => {
        console.log(
          `${JSON.stringify(
            speakers
          )} speakerNumber${speakerNumber} totalVolume${totalVolume}`
        );
      }
    );
    ...
    rtcEngine.on(
      'audioPublishStateChange',
      (channel, oldstate, newstate, elapsed) => {
        console.log(
          `rtcEngine audioPublishStateChange   channel: ${channel},  oldstate:${oldstate},  newstate:${newstate},  elapsed:${elapsed}`
        );
      }
    );
    rtcEngine.on(
      ...
      (channel, oldstate, newstate, elapsed) => {
        console.log(
          `rtcEngine videoPublishStateChange   channel: ${channel},  oldstate:${oldstate},  newstate:${newstate}, elapsed:${elapsed}`
        );
      }
    );
    rtcEngine.on(
      'audioSubscribeStateChange',
      (channel, uid, oldstate, newstate, elapsed) => {
        console.log(
          `rtcEngine audioSubscribeStateChange   channel: ${channel}, uid:${uid}, oldstate:${oldstate},  newstate:${newstate}, elapsed:${elapsed}`
        );
      }
    );
    rtcEngine.on(
      'videoSubscribeStateChange',
      (channel, uid, oldstate, newstate, elapsed) => {
        console.log(
          `rtcEngine videoSubscribeStateChange   channel: ${channel}, uid:${uid}, oldstate:${oldstate}, newstate:${newstate}, elapsed:${elapsed}`
        );
      }
    );
  ...
  subscribeChannelEvents = (rtcChannel, publish) => {
    ...
    rtcChannel.on('userOffline', (uid, reason) => {
      this.setState({
        users: this.state.users.delete(
          this.state.users.indexOf({ channelId, uid })
        ),
      });
    });
    ...
    rtcChannel.on('audioPublishStateChange', (oldstate, newstate, elapsed) => {
      console.log(
        `audioPublishStateChange  oldstate:${oldstate},  newstate:${newstate},  elapsed:${elapsed}`
      );
    });
    rtcChannel.on('videoPublishStateChange', (oldstate, newstate, elapsed) => {
      console.log(
        `videoPublishStateChange  oldstate:${oldstate},  newstate:${newstate}, elapsed:${elapsed}`
      );
    });
    rtcChannel.on(
      'audioSubscribeStateChange',
      (uid, oldstate, newstate, elapsed) => {
        console.log(
          `audioSubscribeStateChange  uid:${uid}, oldstate:${oldstate},  newstate:${newstate}, elapsed:${elapsed}`
        );
      }
    );
    rtcChannel.on(
      ...
      (uid, oldstate, newstate, elapsed) => {
        console.log(
          `videoSubscribeStateChange uid:${uid}, oldstate:${oldstate}, newstate:${newstate}, elapsed:${elapsed}`
        );
      }
    );
    rtcChannel.on('receiveMetadata', metadata => {
      console.log(
        `channel receiveMetadata  ${metadata.uid}  size: ${metadata.size}  buffer: ${metadata.buffer}  timeStampMs: ${metadata.timeStampMs}`
      );
    });

    rtcChannel.on('sendMetadataSuccess', metadata => {
      console.log(
        `channel sendMetadataSuccess  ${metadata.uid}  size: ${metadata.size}  buffer: ${metadata.buffer}  timeStampMs: ${metadata.timeStampMs}`
      );
    });
  };
  ...
  handleJoin = () => {
    ...
    rtcEngine.setParameters(
      '{"rtc.user_account_server_list":["58.211.82.170"]}'
    );
  ...
  toggleFuPlugin = () => {
    const plugin = this.rtcEngine
      .getPlugins()
      .find(plugin => plugin.id === 'bytedance');
    if (plugin) {
    ...
    if (plugin) {
      if (this.state.bdEnabled) {
        ...
        if (isMac) {
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.licensePath': path.join(
                __static,
                'bytedance/resource/license.licbag'
              ),
            })
          );
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.stickerPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle'
              ),
            })
          );
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.beauty.resourcepath': path.join(
                __static,
                'bytedance/resource/BeautyResource.bundle/IESBeauty'
              ),
            })
          );
          plugin.setParameter(
          ...
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.faceDetectModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/ttfacemodel/tt_face_v6.0.model'
              ),
              'plugin.bytedance.faceDetectExtraModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/ttfacemodel/tt_face_extra_v9.0.model'
              ),
              'plugin.bytedance.faceAttributeModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/ttfaceattri/tt_face_attribute_v4.1.model'
              ),
          ...
          plugin.setParameter(
            JSON.stringify({
              ...
              'plugin.bytedance.handDetectModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/handmodel/tt_hand_det_v9.0.model'
              ),
              'plugin.bytedance.handBoxModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/handmodel/tt_hand_box_reg_v10.0.model'
              ),
              'plugin.bytedance.handGestureModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/handmodel/tt_hand_gesture_v8.1.model'
              ),
              'plugin.bytedance.handKPModelPath': path.join(
                __static,
                'bytedance/resource/StickerResource.bundle/handmodel/tt_hand_kp_v5.0.model'
              ),
            })
          );
          ...
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.licensePath': path.join(
                __static,
                'bytedance/resource/license.bag'
              ),
            })
          );
          ...
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.faceDetectModelPath': path.join(
                __static,
                'bytedance/resource/model/ttfacemodel/tt_face_v6.0.model'
              ),
            })
          );
          plugin.setParameter(
            JSON.stringify({
              'plugin.bytedance.faceAttributeModelPath': path.join(
                __static,
                'bytedance/resource/model/ttfaceattrmodel/tt_face_attribute_v4.1.model'
              ),
            })
          );
        }
  ...
  onPressTest = () => {
    const engine = this.getRtcEngine();
    engine.takeSnapshot('123', 0, './sadasd');
    engine.startRtmpStreamWithTranscoding('testChannel', {
      /** width of canvas */
      width: 640,
      /** height of canvas */
      height: 480,
      ...
      audioCodecProfile: 0,
      advancedFeatures: [
        { featureName: 'featureName1', opened: true },
        { featureName: 'featureName2', opened: true },
      ],
    });
    engine.startRtmpStreamWithoutTranscoding('asdas');
    engine.setAVSyncSource('asdas', 1);
    engine.followSystemPlaybackDevice(true);
    engine.followSystemRecordingDevice(true);

    engine.videoSourceDisableAudio();
    engine.adjustLoopbackSignalVolume(40);
    engine.videoSourceAdjustRecordingSignalVolume(50);
    engine.videoSourceAdjustLoopbackRecordingSignalVolume(60);
    engine.videoSourceMuteRemoteAudioStream(2, true);
    engine.videoSourceMuteAllRemoteAudioStreams(true);
    engine.videoSourceMuteRemoteVideoStream(3, true);
    engine.videoSourceMuteAllRemoteVideoStreams(true);
    let list = engine.getDefaultAudioPlaybackDevices();
  ...
  handleAudioHook = e => {
    ...
    if (!this.state.audioHookEnabled) {
      ...
      let dllpath = path.resolve(
        __dirname,
        './plugins/AgoraPlayerHookPlugin.dll'
      );
      rtcEngine.loadPlugin('agora_electron_plugin_audio_hook', dllpath);
      let playerpath = path.resolve(this.state.hookplayerpath);
      rtcEngine.setPluginStringParameter(
        'agora_electron_plugin_audio_hook',
        'plugin.hookAudio.playerPath',
        playerpath
      );
      rtcEngine.setPluginBoolParameter(
        'agora_electron_plugin_audio_hook',
        'plugin.hookAudio.forceRestart',
        true
      );
      ...
      console.log(
        rtcEngine.unRegisterAudioFramePlugin('agora_electron_plugin_audio_hook')
      );
    ...
    return (
      ...
      >
        ...
        <div className="column is-one-quarter" style={{ overflowY: 'auto' }}>
          <div className="field">{`pid:${process.pid}`}</div>
          <div className="field">
          ...
          <div className="field">
            ...
            <div className="control">
              <input
                onChange={e =>
                  this.setState({ encoderWidth: e.currentTarget.value })
                }
                value={this.state.encoderWidth}
                className="input"
                type="number"
                placeholder="Encoder Width"
              />
              <input
                onChange={e =>
                  this.setState({ encoderHeight: e.currentTarget.value })
                }
                value={this.state.encoderHeight}
                className="input"
                type="number"
                placeholder="Encoder Height"
              />
            </div>
          </div>
          <div className="field">
          ...
          <div className="field">
            ...
            <div className="control">
              <div className="select" style={{ width: '100%' }}>
                <select
                  onChange={this.handleVideoProfile}
                  value={this.state.videoProfile}
                  style={{ width: '100%' }}
                >
          ...
          <div className="field">
            ...
            <div className="control">
              <button
                onClick={this.updateScreenShareParam}
                className="button is-link"
              >
                Update Screen Share
              </button>
          ...
          <div className="field">
            ...
            <div className="control">
              <input
                onChange={e =>
                  this.setState({ hookplayerpath: e.currentTarget.value })
                }
                value={this.state.hookplayerpath}
                className="input"
                type="text"
                placeholder="Absolute player path"
              />
            </div>
          </div>
          <div className="field">
            ...
            <div className="control">
              <button onClick={this.handleAudioHook} className="button is-link">
                Start
              </button>
            </div>
          </div>
          ...
          <div className="field">
            ...
            <div className="control">
              <button onClick={this.toggleFuPlugin} className="button is-link">
                {this.state.fuEnabled ? 'disable' : 'enable'}
              </button>
            </div>
          </div>
          <div className="field">
            ...
            <div className="control">
              <button
                onClick={this.toggleByteDancePlugin}
                className="button is-link"
              >
                {this.state.bdEnabled ? 'disable' : 'enable'}
              </button>
            </div>
          </div>
          <div className="field">
            <label className="label">Test Field</label>
            <div className="control">
              <button onClick={this.onPressTest} className="button is-link">
                Test
              </button>
            </div>
          </div>
        ...
        <div className="column is-three-quarters window-container">
          ...
          {this.state.localVideoSource ? (
            <Window
              uid={this.state.localVideoSource}
              rtcEngine={this.rtcEngine}
...
class Window extends Component {
  ...
  componentDidMount() {
    let dom = document.querySelector(
      `#video-${this.props.channel || ''}-${this.props.uid}`
    );
    ...
    return (
      <div className="window-item">
        <div
          ...
          id={`video-${this.props.channel || ''}-${this.props.uid}`}
        ></div>
      </div>
    );
  }
