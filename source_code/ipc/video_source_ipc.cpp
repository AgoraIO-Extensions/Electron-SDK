...
bool AgoraVideoSourceIpcImpl::connect() {
  ...
  if (m_ipc.open_channel(AgoraVideoSourceIpcImpl::s_ipcSourceWriteChannelId,
                         CHANNEL_READ) != 0) {
    m_ipc.close();
    return false;
  }

  if (m_ipc.open_channel(AgoraVideoSourceIpcImpl::s_ipcSourceReadChannelId,
                         CHANNEL_WRITE) != 0) {
...
bool AgoraVideoSourceIpcImpl::disconnect() {
  m_ipc.close_channel(AgoraVideoSourceIpcImpl::s_ipcSourceReadChannelId,
  ...
  while (m_initialize) {
    ...
    if (m_listener) {
      ...
      m_listener->OnMessage(
          header->msg, readBuf + sizeof(VideoSourceIpcMsgHeader), header->len);
    }
  }
  delete[] readBuf;
}

bool AgoraVideoSourceIpcImpl::sendMessage(AgoraIpcMsg msg, char *payload,
                                          unsigned int len) {
  uint32_t fd = m_ipcOwner ? AgoraVideoSourceIpcImpl::s_ipcSourceWriteChannelId
                           : AgoraVideoSourceIpcImpl::s_ipcSourceReadChannelId;
  VideoSourceIpcMsgHeader hdr = {msg, len};
  std::vector<std::pair<char *, int32_t>> data;
  data.push_back(
      std::make_pair<char *, unsigned int>((char *)&hdr, sizeof(hdr)));
  data.push_back(
      std::make_pair<char *, unsigned int>(std::move(payload), std::move(len)));
...
AgoraIpcDataSender::AgoraIpcDataSender() : m_ipcData(), m_initialized(false) {}
...
bool AgoraIpcDataSender::initialize(const std::string &id) {
  ...
  if (m_ipcData.create(id, 1) != 0) {
    LOG_F(INFO, "%s, ipc data create fail, errno :%d\n", __FUNCTION__, errno);

    return false;
  }
  if (m_ipcData.open(id) != 0) {
    LOG_F(INFO, "%s, ipc data open fail, errno :%d\n", __FUNCTION__, errno);
    m_ipcData.remove(id);
  ...
  if (m_ipcData.open_channel(0, CHANNEL_WRITE) != 0) {
    m_ipcData.close();
    m_ipcData.remove(id);
    LOG_F(INFO, "%s, ipc data open channel fail, errno :%d\n", __FUNCTION__,
          errno);
...
void AgoraIpcDataSender::sendMultiData(
    const std::vector<std::pair<char *, int32_t>> &payloads) {
...
AgoraIpcDataReceiver::~AgoraIpcDataReceiver() { stop(); }

bool AgoraIpcDataReceiver::initialize(
    const std::string &id,
