...
enum CHANNEL_OPEN_MODE { CHANNEL_READ = 1, CHANNEL_WRITE };
...
public:
  ...
  void *address() override { return addr; }

  int64_t size() override { return region_size; }
...
#define AtomicAddAndFetch(x, y)                                                \
  (InterlockedExchangeAdd64((LONG64 *)(x), (y)) + (y))
...
public:
  win32_shm()
      : creator_file_handle(NULL), open_file_handle(NULL), region_size(0),
        addr(NULL) {}

  ~win32_shm() { close(); }

  int create(const std::string &name, int64_t size) override {
    creator_file_handle = ::CreateFileMappingA(NULL, NULL, PAGE_READWRITE, 0,
  ...
  int open(const std::string &name) override {
    open_file_handle =
        ::OpenFileMappingA(FILE_MAP_READ | FILE_MAP_WRITE, FALSE, name.c_str());
    if (open_file_handle == NULL)
      return -1;

    addr = ::MapViewOfFile(open_file_handle, FILE_MAP_READ | FILE_MAP_WRITE, 0,
                           0, 0);
    if (addr == NULL) {
      close();
      return -1;
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE> class shm_ipc {
public:
  shm_ipc();
  virtual ~shm_ipc();
   ...
   *		 this function will wait until reader pop up some data, no
   *matter read end opened or not
   */
   ...
   *		 this function will wait until writer push some some data, no
   *matter write end opened or not
   */
...
template <uint32_t BLOCK_SIZE> struct shm_block {
  int64_t lock;
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE> struct shm_channel {
  int64_t used;
  int64_t read_opened;
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE> struct shm_header {
  int64_t size;
...
static shm_channel<BLOCK_NUM, BLOCK_SIZE> *
shm_block_get_channel(shm_block<BLOCK_SIZE> *block) {
  unsigned char *addr = (unsigned char *)block;
  shm_channel<BLOCK_NUM, BLOCK_SIZE> *channel =
      (shm_channel<BLOCK_NUM, BLOCK_SIZE> *)(addr - block->channel_offset);
  return channel;
}

template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
static shm_header<BLOCK_NUM, BLOCK_SIZE> *
shm_channel_get_header(shm_channel<BLOCK_NUM, BLOCK_SIZE> *channel) {
  unsigned char *addr = (unsigned char *)channel;
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE> *)(addr - channel->header_offset);
  return header;
}
...
static int shm_block_lock(shm_block<BLOCK_SIZE> *block) {
  ...
  while (AtomicTestAndSet(&block->lock, 1) == 1) {
    ...
    if (AtomicGet(
            &(shm_block_get_channel<BLOCK_NUM, BLOCK_SIZE>(block)->used)) == 0)
      return -1;
  ...
  // if channel not available during wait, return error
  if (AtomicGet(&(shm_block_get_channel<BLOCK_NUM, BLOCK_SIZE>(block)->used)) ==
      0) {
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
static int shm_block_wait_for_data_state(shm_block<BLOCK_SIZE> *block,
                                         int64_t state) {
  int retry_count = 0;
  shm_channel<BLOCK_NUM, BLOCK_SIZE> *channel =
      shm_block_get_channel<BLOCK_NUM, BLOCK_SIZE>(block);
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      shm_channel_get_header<BLOCK_NUM, BLOCK_SIZE>(channel);
 ...
 */
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
static void init_channel(shm_header<BLOCK_NUM, BLOCK_SIZE> *header,
                         shm_channel<BLOCK_NUM, BLOCK_SIZE> *channel) {
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
int shm_ipc<BLOCK_NUM, BLOCK_SIZE>::create(const std::string &name,
                                           int64_t channel_count) {
  // creator needs to initialize the share buffer
  int64_t total_size =
      sizeof(shm_header<BLOCK_NUM, BLOCK_SIZE>) +
  ...
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE> *)shmOp->address();
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
int shm_ipc<BLOCK_NUM, BLOCK_SIZE>::open_channel(int id,
                                                 CHANNEL_OPEN_MODE mode) {
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE> *)shmOp->address();
  ...
  if ((mode == CHANNEL_READ) && AtomicGet(&header->channels[id].read_opened))
    return -1;

  if ((mode == CHANNEL_WRITE) && AtomicGet(&header->channels[id].write_opened))
    return -1;
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
int shm_ipc<BLOCK_NUM, BLOCK_SIZE>::close_channel(int id,
                                                  CHANNEL_OPEN_MODE mode) {
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE> *)shmOp->address();
  if (!header)
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
int32_t shm_ipc<BLOCK_NUM, BLOCK_SIZE>::write(int fd, const void *buf,
                                              int32_t size) {
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE> *)shmOp->address();
  ...
  if (AtomicGet(&channel->used) != 1 || AtomicGet(&channel->write_opened) != 1)
    return -1;
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
int32_t shm_ipc<BLOCK_NUM, BLOCK_SIZE>::write(
    int fd, const std::vector<std::pair<char *, int32_t>> &bufs) {
  shm_header<BLOCK_NUM, BLOCK_SIZE> *header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE> *)shmOp->address();
  if (fd < 0 || fd > header->channel_count)
    return -1;
...
int32_t shm_ipc<BLOCK_NUM, BLOCK_SIZE>::read(int fd, void *buf, int32_t size) {
  ...
  shm_channel<BLOCK_NUM, BLOCK_SIZE> *channel = &header->channels[fd];
  if (AtomicGet(&channel->used) != 1 || AtomicGet(&channel->read_opened) != 1)
    return -1;
