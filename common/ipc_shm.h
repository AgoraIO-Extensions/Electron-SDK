...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
static shm_channel<BLOCK_NUM, BLOCK_SIZE>* shm_block_get_channel(
    shm_block<BLOCK_SIZE>* block) {
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
static shm_header<BLOCK_NUM, BLOCK_SIZE>* shm_channel_get_header(
    shm_channel<BLOCK_NUM, BLOCK_SIZE>* channel) {
...
template <uint32_t BLOCK_NUM, uint32_t BLOCK_SIZE>
int32_t shm_ipc<BLOCK_NUM, BLOCK_SIZE>::write(int fd,
                                              const void* buf,
                                              int32_t size) {
  shm_header<BLOCK_NUM, BLOCK_SIZE>* header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE>*)shmOp->address();
...
int32_t shm_ipc<BLOCK_NUM, BLOCK_SIZE>::write(
    int fd,
    const std::vector<std::pair<char*, int32_t>>& bufs) {
  shm_header<BLOCK_NUM, BLOCK_SIZE>* header =
      (shm_header<BLOCK_NUM, BLOCK_SIZE>*)shmOp->address();
  if (fd < 0 || (header != NULL && fd > header->channel_count)) {
    return -1;
