...
void NodeEventHandler::onRejoinChannelSuccess(const char* channel, uid_t uid, int elapsed) {
  ...
  std::string channelName = channel;
  node_async_call::async_call([this, channelName, uid, elapsed]() {
    MAKE_JS_CALL_3(RTC_EVENT_REJOIN_CHANNEL, string, channelName.c_str(), uid, uid, int32, elapsed);
    });
}

void NodeEventHandler::onAudioQuality(uid_t uid, int quality, unsigned short delay, unsigned short lost) {
  FUNC_TRACE;
  node_async_call::async_call([this, uid, quality, delay, lost] {
    MAKE_JS_CALL_4(RTC_EVENT_AUDIO_QUALITY, uid, uid, int32, quality, uint32, delay, uint32, lost);
    });
}
...
void NodeEventHandler::onAudioVolumeIndication(const AudioVolumeInfo* speakers, unsigned int speakerNumber, int totalVolume)
{
  FUNC_TRACE;
  if (speakers) {
    AudioVolumeInfo* localSpeakers = new AudioVolumeInfo[speakerNumber];
    for (unsigned int i = 0; i < speakerNumber; i++) {
      AudioVolumeInfo tmp = speakers[i];
...
void NodeEventHandler::onNetworkQuality(uid_t uid, int txQuality, int rxQuality)
{
  ...
  //  MAKE_JS_CALL_3(RTC_EVENT_NETWORK_QUALITY, uint32, uid, int32, txQuality, int32 rxQuality);
  //  });
}

void NodeEventHandler::onIntraRequestReceived() {
  FUNC_TRACE;
  node_async_call::async_call([this] {
    MAKE_JS_CALL_0(RTC_EVENT_INTRAREQUESTRECEIVED);
    });
}

void NodeEventHandler::onUplinkNetworkInfoUpdated(const UplinkNetworkInfo& info)
{
  FUNC_TRACE;
  do {
    Isolate* isolate = Isolate::GetCurrent();
    ...
    Local<Object> obj = Object::New(isolate);
    CHECK_NAPI_OBJ(obj);
    NODE_SET_OBJ_PROP_UINT32(obj, "video_encoder_target_bitrate_bps", info.video_encoder_target_bitrate_bps);

    Local<Value> arg[1] = { obj };
    auto it = m_callbacks.find(RTC_EVENT_INTRAREQUESTRECEIVED);
    if (it != m_callbacks.end()) {
      it->second->callback.Get(isolate)->Call(context, it->second->js_this.Get(isolate), 1, arg);
    }
  } while (false);
...
void NodeEventHandler::onDownlinkNetworkInfoUpdated(const DownlinkNetworkInfo& info)
{
  FUNC_TRACE;
  ...
  do {
    ...
    Local<Object> obj = Object::New(isolate);
    CHECK_NAPI_OBJ(obj);
    NODE_SET_OBJ_PROP_UINT32(obj, "bandwidth_estimation_bps", info.bandwidth_estimation_bps);
    NODE_SET_OBJ_PROP_UINT32(obj, "total_downscale_level_count", info.total_downscale_level_count);

    //NODE_SET_OBJ_PROP_UINT32(obj, "peer_downlink_info", info.peer_downlink_info); // todo
    NODE_SET_OBJ_PROP_UINT32(obj, "total_received_video_count", info.total_received_video_count);
...
void NodeEventHandler::onVideoSourceFrameSizeChanged(VIDEO_SOURCE_TYPE sourceType, int width, int height)
{
  ...
  node_async_call::async_call([this, sourceType, width, height] {
    MAKE_JS_CALL_3(RTC_EVENT_VIDEO_SOURCE_FRAME_SIZE_CHANGED, uint32, sourceType, int32, width, int32, height);
    });
}
...
void NodeEventHandler::onLocalVideoStateChanged(LOCAL_VIDEO_STREAM_STATE state, LOCAL_VIDEO_STREAM_ERROR error)
{
  FUNC_TRACE;
  node_async_call::async_call([this, state, error] {
    MAKE_JS_CALL_2(RTC_EVENT_LOCAL_VIDEO_STATE_CHANGED, uint32, state, uint32, error);
    });
}
...
void NodeEventHandler::onRemoteVideoStateChanged(uid_t uid, REMOTE_VIDEO_STATE state, REMOTE_VIDEO_STATE_REASON reason, int elapsed)
{
  ...
  node_async_call::async_call([this, uid, state, reason, elapsed] {
    MAKE_JS_CALL_4(RTC_EVENT_REMOTE_VIDEO_STATE_CHANGED, uid, uid, uint32, state, uint32, reason, int32, elapsed);
    });
}

void NodeEventHandler::onFirstRemoteVideoFrame(uid_t userId, int width, int height, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, userId, width, height, elapsed] {
    MAKE_JS_CALL_4(RTC_EVENT_FIRST_REMOTE_VIDEO_FRAME, uid, userId, int32, width, int32, height, int32, elapsed);
    });
}
...
void NodeEventHandler::onUserOffline(uid_t uid, USER_OFFLINE_REASON_TYPE reason)
{
  ...
  node_async_call::async_call([this, uid, reason] {
    MAKE_JS_CALL_2(RTC_EVENT_USER_OFFLINE, uid, uid, uint32, reason);
    });
}

void NodeEventHandler::onUserMuteVideo(uid_t userId, bool muted)
{
  FUNC_TRACE;
  node_async_call::async_call([this, userId, muted] {
    MAKE_JS_CALL_2(RTC_EVENT_USER_MUTE_VIDEO, uid, userId, bool, muted);
    });
}
...
virtual void NodeEventHandler::onFacePositionChanged(int imageWidth, int imageHeight,
...
void NodeEventHandler::onAudioMixingStateChanged(AUDIO_MIXING_STATE_TYPE state, AUDIO_MIXING_ERROR_TYPE errorCode)
{
  ...
  node_async_call::async_call([this, state, errorCode] {
    MAKE_JS_CALL_2(RTC_EVENT_AUDIO_MIXING_STATE_CHANGED, uint32, state, uint32, errorCode);
    });
}
...
void NodeEventHandler::onStreamMessage(uid_t userId, int streamId, const char* data, size_t length, uint64_t sentTs)
{
  FUNC_TRACE;
  std::string msg(data);
  node_async_call::async_call([this, userId, streamId, msg, length, sentTs] {
    MAKE_JS_CALL_5(RTC_EVENT_STREAM_MESSAGE, uid, userId, int32, streamId, string, msg.c_str(), int32, length, uint64, sentTs);
    });
}

void NodeEventHandler::onStreamMessageError( uid_t userId, int streamId, int code, int missed, int cached)
{
  FUNC_TRACE;
  node_async_call::async_call([this, userId, streamId, code, missed, cached] {
    MAKE_JS_CALL_5(RTC_EVENT_STREAM_MESSAGE_ERROR, uid, userId, int32, streamId, int32, code, int32, missed, int32, cached);
    });
}
...
void NodeEventHandler::onTokenPrivilegeWillExpire(const char* token)
{
  ...
  node_async_call::async_call([this, sToken] {
    MAKE_JS_CALL_1(RTC_EVENT_TOKEN_PRIVILEGE_WILL_EXPIRE, string, sToken.c_str());
    });
}
...
void NodeEventHandler::onLocalAudioStateChanged(LOCAL_AUDIO_STREAM_STATE state, LOCAL_AUDIO_STREAM_ERROR error)
{
  FUNC_TRACE;
  node_async_call::async_call([this, state, error] {
    MAKE_JS_CALL_2(RTC_EVENT_LOCAL_AUDIO_STATE_CHANGED, uint32, state, uint32, error);
    });
}
...
void NodeEventHandler::onRemoteAudioStateChanged(uid_t uid, REMOTE_AUDIO_STATE state, REMOTE_AUDIO_STATE_REASON reason, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, uid, state, reason, elapsed] {
    MAKE_JS_CALL_4(RTC_EVENT_REMOTE_AUDIO_STATE_CHANGED, uid, uid, uint32, state, uint32, reason, int32, elapsed);
    });
}

void NodeEventHandler::onActiveSpeaker(uid_t userId)
{
  FUNC_TRACE;
  node_async_call::async_call([this, userId] {
    MAKE_JS_CALL_1(RTC_EVENT_ACTIVE_SPEAKER, uid, userId);
    });
}
...
void NodeEventHandler::onClientRoleChanged(CLIENT_ROLE_TYPE oldRole, CLIENT_ROLE_TYPE newRole)
{
  ...
  node_async_call::async_call([this, oldRole, newRole] {
    MAKE_JS_CALL_2(RTC_EVENT_CLIENT_ROLE_CHANGED, uint32, oldRole, uint32, newRole);
    });
}
...
void NodeEventHandler::onAudioDeviceVolumeChanged(MEDIA_DEVICE_TYPE deviceType, int volume, bool muted)
{
  ...
  node_async_call::async_call([this, deviceType, volume, muted] {
    MAKE_JS_CALL_3(RTC_EVENT_AUDIO_DEVICE_VOLUME_CHANGED, uint32, deviceType, int32, volume, bool, muted);
    });
}

void NodeEventHandler::onRtmpStreamingStateChanged(const char* url, RTMP_STREAM_PUBLISH_STATE state, RTMP_STREAM_PUBLISH_ERROR errCode)
{
  FUNC_TRACE;
  std::string sUrl(url);
  node_async_call::async_call([this, sUrl, state, errCode] {
    MAKE_JS_CALL_3(RTC_EVENT_RTMP_STREAMING_STATE_CHANGED, string, sUrl.c_str(), uint32, state, uint32, errCode)
    });
}
...
void NodeEventHandler::onStreamPublished(const char* url, int error)
{
  ...
  node_async_call::async_call([this, sUrl, error] {
    MAKE_JS_CALL_2(RTC_EVENT_STREAM_PUBLISHED, string, sUrl.c_str(), int32, error);
    });
}
...
void NodeEventHandler::onChannelMediaRelayStateChanged(int state, int code)
{
  ...
  node_async_call::async_call([this, state, code] {
    MAKE_JS_CALL_2(RTC_EVENT_CHANNEL_MEDIA_RELAY_STATE_CHANGED, int32, state, int32, code);
    });
}
...
void NodeEventHandler::onRemoteAudioTransportStats(uid_t uid, unsigned short delay, unsigned short lost, unsigned short rxKBitRate)
{
  FUNC_TRACE;
...
void NodeEventHandler::onRemoteVideoTransportStats(uid_t uid, unsigned short delay, unsigned short lost, unsigned short rxKBitRate)
{
  FUNC_TRACE;
  node_async_call::async_call([this, uid, delay, lost, rxKBitRate] {
...
void NodeEventHandler::onConnectionStateChanged(CONNECTION_STATE_TYPE state, CONNECTION_CHANGED_REASON_TYPE reason)
{
  ...
  node_async_call::async_call([this, state, reason] {
    MAKE_JS_CALL_2(RTC_EVENT_CONNECTION_STATE_CHANGED, uint32, state, uint32, reason);
    });
}
...
void NodeEventHandler::onNetworkTypeChanged(NETWORK_TYPE type)
{
  ...
  node_async_call::async_call([this, type] {
    MAKE_JS_CALL_1(RTC_EVENT_NETWORK_TYPE_CHANGED, uint32, type);
    });
}

void NodeEventHandler::onEncryptionError(ENCRYPTION_ERROR_TYPE errorType)
{
  FUNC_TRACE;
  node_async_call::async_call([this, errorType] {
    MAKE_JS_CALL_1(RTC_EVENT_ENCRYPTION_ERROR, uint32, errorType);
    });
}
...
void NodeEventHandler::onLocalUserRegistered(uid_t uid, const char* userAccount)
{
  ...
  node_async_call::async_call([this, uid, mUserAccount] {
    MAKE_JS_CALL_2(RTC_EVENT_LOCAL_USER_REGISTERED, uid, uid, string, mUserAccount.c_str());
    });
}
...
void NodeEventHandler::onAudioSubscribeStateChanged(const char* channel, uid_t uid, STREAM_SUBSCRIBE_STATE oldState, STREAM_SUBSCRIBE_STATE newState, int elapseSinceLastState)
{
  ...
  node_async_call::async_call([this, mChannel, uid, oldState, newState, elapseSinceLastState] {
    MAKE_JS_CALL_5(RTC_EVENT_AUDIO_SUBSCRIBE_STATE_CHANGED, string, mChannel.c_str(), uid, uid, uint32, oldState, uint32, newState, int32, elapseSinceLastState);
    });
}
...
void NodeEventHandler::onVideoSubscribeStateChanged(const char* channel, uid_t uid, STREAM_SUBSCRIBE_STATE oldState, STREAM_SUBSCRIBE_STATE newState, int elapseSinceLastState)
{
  ...
  std::string mChannel(channel);
  node_async_call::async_call([this, mChannel, uid, oldState, newState, elapseSinceLastState] {
    MAKE_JS_CALL_5(RTC_EVENT_VIDEO_SUBSCRIBE_STATE_CHANGED, string, mChannel.c_str(), uid, uid, uint32, oldState, uint32, newState, int32, elapseSinceLastState);
    });
}
...
void NodeEventHandler::onAudioPublishStateChanged(const char* channel, STREAM_PUBLISH_STATE oldState, STREAM_PUBLISH_STATE newState, int elapseSinceLastState)
{
  ...
  node_async_call::async_call([this, mChannel, oldState, newState, elapseSinceLastState] {
    MAKE_JS_CALL_4(RTC_EVENT_AUDIO_PUBLISH_STATE_CHANGED, string, mChannel.c_str(), uint32, oldState, uint32, newState, int32, elapseSinceLastState);
    });
}
...
void NodeEventHandler::onVideoPublishStateChanged(const char* channel, STREAM_PUBLISH_STATE oldState, STREAM_PUBLISH_STATE newState, int elapseSinceLastState)
{
  ...
  std::string mChannel(channel);
  node_async_call::async_call([this, mChannel, oldState, newState, elapseSinceLastState] {
    MAKE_JS_CALL_4(RTC_EVENT_VIDEO_PUBLISH_STATE_CHANGED, string, mChannel.c_str(), uint32, oldState, uint32, newState, int32, elapseSinceLastState);
    });
}

void NodeEventHandler::onExtensionEvent(const char* provider_name, const char* ext_name, const char* key, const char* json_value) {
  FUNC_TRACE;
  std::string sProviderName(provider_name);
  std::string sExtName(ext_name);
  std::string sKey(key);
  std::string sJsonValue(json_value);
  node_async_call::async_call([this, sProviderName, sExtName, sKey, sJsonValue] {
    MAKE_JS_CALL_4(RTC_EVENT_EXTENSION_EVENT, string, sProviderName.c_str(), string, sExtName.c_str(), string, sKey.c_str(), string, sJsonValue.c_str());
    });
}

void NodeEventHandler::onExtensionStarted(const char* provider_name, const char* ext_name)
{
  FUNC_TRACE;
  std::string sProviderName(provider_name);
  std::string sExtName(ext_name);
  node_async_call::async_call([this, sProviderName, sExtName] {
    MAKE_JS_CALL_2(RTC_EVENT_EXTENSION_STARTED, string, sProviderName.c_str(), string, sExtName.c_str());
    });
}

void NodeEventHandler::onExtensionStopped(const char* provider_name, const char* ext_name)
{
  FUNC_TRACE;
  std::string sProviderName(provider_name);
  std::string sExtName(ext_name);
  node_async_call::async_call([this, sProviderName, sExtName] {
    MAKE_JS_CALL_2(RTC_EVENT_EXTENSION_STOPPED, string, sProviderName.c_str(), string, sExtName.c_str());
    });
}

void NodeEventHandler::onExtensionErrored(const char* provider_name, const char* ext_name, int error, const char* msg)
{
  FUNC_TRACE;
  std::string sProviderName(provider_name);
  std::string sExtName(ext_name);
  ...
  node_async_call::async_call([this, sProviderName, sExtName, sMsg] {
    MAKE_JS_CALL_3(RTC_EVENT_EXTENSION_ERRORED, string, sProviderName.c_str(), string, sExtName.c_str(), string, sMsg.c_str());
    });
}

void NodeEventHandler::onUserAccountUpdated(uid_t uid, const char* userAccount)
{
  FUNC_TRACE;
  std::string sUserAccount(userAccount);
  node_async_call::async_call([this, uid, sUserAccount] {
    MAKE_JS_CALL_2(RTC_EVENT_USER_ACCOUNT_UPDATED, uid, uid, string, sUserAccount.c_str());
    });
}
...
void NodeEventHandler::onRejoinChannelSuccess(const RtcConnection& connection, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
...
void NodeEventHandler::onAudioQuality(const RtcConnection& connection, uid_t remoteUid, int quality, unsigned short delay, unsigned short lost)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, quality, delay, lost] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_AUDIO_QUALITY,
      5,
      connection,
...
void NodeEventHandler::onNetworkQuality(const RtcConnection& connection, uid_t remoteUid, int txQuality, int rxQuality)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, txQuality, rxQuality] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_INTRAREQUESTRECEIVED,
      1,
      connection
    );
    });
...
void NodeEventHandler::onFirstLocalVideoFrame(const RtcConnection& connection, int width, int height, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, width, height, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_FIRST_LOCAL_VIDEO_FRAME,
      4,
      connection,
...
void NodeEventHandler::onFirstLocalVideoFramePublished(const RtcConnection& connection, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_FIRST_LOCAL_VIDEO_FRAME_PUBLISH,
      2,
      connection,
      napi_create_int32_(isolate, elapsed)
    );
...
void NodeEventHandler::onVideoSourceFrameSizeChanged(const RtcConnection& connection, VIDEO_SOURCE_TYPE sourceType, int width, int height)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, sourceType, width, height] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_VIDEO_SOURCE_FRAME_SIZE_CHANGED,
      4,
      connection,
...
void NodeEventHandler::onFirstRemoteVideoDecoded(const RtcConnection& connection, uid_t remoteUid, int width, int height, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, width, height, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onVideoSizeChanged(const RtcConnection& connection, uid_t uid, int width, int height, int rotation)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, uid, width, height, rotation] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onLocalVideoStateChanged(const RtcConnection& connection, LOCAL_VIDEO_STREAM_STATE state, LOCAL_VIDEO_STREAM_ERROR errorCode)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, state, errorCode] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onRemoteVideoStateChanged(const RtcConnection& connection, uid_t remoteUid, REMOTE_VIDEO_STATE state, REMOTE_VIDEO_STATE_REASON reason, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, state, reason] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onFirstRemoteVideoFrame(const RtcConnection& connection, uid_t remoteUid, int width, int height, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, width, height, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_FIRST_REMOTE_VIDEO_FRAME,
      5,
      connection,
      napi_create_uid_(isolate, remoteUid),
      napi_create_int32_(isolate, width),
...
void NodeEventHandler::onUserMuteVideo(const RtcConnection& connection, uid_t remoteUid, bool muted)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, muted] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_USER_MUTE_VIDEO,
      3,
      connection,
      napi_create_uid_(isolate, remoteUid),
...
void NodeEventHandler::onUserEnableVideo(const RtcConnection& connection, uid_t remoteUid, bool enabled)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, enabled] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_USER_ENABLE_VIDEO,
      3,
      connection,
      napi_create_uid_(isolate, remoteUid),
...
void NodeEventHandler::onUserEnableLocalVideo(const RtcConnection& connection, uid_t remoteUid, bool enabled)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, enabled] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_USER_ENABLE_LOCAL_VIDEO,
      3,
      connection,
      napi_create_uid_(isolate, remoteUid),
      napi_create_bool_(isolate, enabled)
...
void NodeEventHandler::onConnectionLost(const RtcConnection& connection)
{
  ...
  node_async_call::async_call([this, connection] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_CONNECTION_LOST,
      1,
      connection
    );
    });
...
void NodeEventHandler::onConnectionInterrupted(const RtcConnection& connection)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_CONNECTION_INTERRUPTED,
      1,
      connection
    );
    });
...
void NodeEventHandler::onConnectionBanned(const RtcConnection& connection)
{
  ...
  node_async_call::async_call([this, connection] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_CONNECTION_BANNED,
      1,
      connection
    );
    });
...
void NodeEventHandler::onStreamMessage(const RtcConnection& connection, uid_t remoteUid, int streamId, const char* data, size_t length, uint64_t sentTs)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, streamId, data, length, sentTs] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_STREAM_MESSAGE,
      6,
      connection,
      napi_create_uid_(isolate, remoteUid),
...
void NodeEventHandler::onStreamMessageError(const RtcConnection& connection, uid_t remoteUid, int streamId, int code, int missed, int cached)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, streamId, code, missed, cached] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_STREAM_MESSAGE_ERROR,
      6,
      connection,
      napi_create_uid_(isolate, remoteUid),
      napi_create_int32_(isolate, streamId),
...
void NodeEventHandler::onRequestToken(const RtcConnection& connection)
{
  ...
  node_async_call::async_call([this, connection] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_REQUEST_TOKEN,
      1,
      connection
    );
    });
...
void NodeEventHandler::onTokenPrivilegeWillExpire(const RtcConnection& connection, const char* token)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, token] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_TOKEN_PRIVILEGE_WILL_EXPIRE,
      2,
      connection,
...
void NodeEventHandler::onFirstLocalAudioFramePublished(const RtcConnection& connection, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_FIRST_LOCAL_AUDIO_FRAME_PUBLISH,
      2,
      connection,
      napi_create_int32_(isolate, elapsed)
    );
...
void NodeEventHandler::onRemoteAudioStateChanged(const RtcConnection& connection, uid_t remoteUid, REMOTE_AUDIO_STATE state, REMOTE_AUDIO_STATE_REASON reason, int elapsed)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, state, reason, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onActiveSpeaker(const RtcConnection& connection, uid_t uid)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, uid] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_ACTIVE_SPEAKER,
      2,
      connection,
...
void NodeEventHandler::onClientRoleChanged(const RtcConnection& connection, CLIENT_ROLE_TYPE oldRole, CLIENT_ROLE_TYPE newRole)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, oldRole, newRole] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_CLIENT_ROLE_CHANGED,
      3,
      connection,
...
void NodeEventHandler::onRemoteAudioTransportStats(const RtcConnection& connection, uid_t remoteUid, unsigned short delay, unsigned short lost,
  unsigned short rxKBitRate)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, delay, lost, rxKBitRate] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_REMOTE_AUDIO_TRANSPORT_STATS,
      5,
      connection,
      napi_create_uid_(isolate, remoteUid),
      napi_create_uint32_(isolate, delay),
...
void NodeEventHandler::onRemoteVideoTransportStats(const RtcConnection& connection, uid_t remoteUid, unsigned short delay, unsigned short lost,
  unsigned short rxKBitRate)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, delay, lost, rxKBitRate] {
    ...
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_REMOTE_VIDEO_TRANSPORT_STATS,
      5,
      connection,
      napi_create_uid_(isolate, remoteUid),
      napi_create_uint32_(isolate, delay),
...
void NodeEventHandler::onConnectionStateChanged(const RtcConnection& connection, CONNECTION_STATE_TYPE state, CONNECTION_CHANGED_REASON_TYPE reason)
{
  ...
  node_async_call::async_call([this, connection, state, reason] {
    ...
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_CONNECTION_STATE_CHANGED,
      3,
      connection,
      napi_create_uint32_(isolate, state),
      napi_create_uint32_(isolate, reason)
...
void NodeEventHandler::onNetworkTypeChanged(const RtcConnection& connection, NETWORK_TYPE type)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, type] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_NETWORK_TYPE_CHANGED,
      2,
      connection,
...
void NodeEventHandler::onEncryptionError(const RtcConnection& connection, ENCRYPTION_ERROR_TYPE errorType)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, errorType] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_ENCRYPTION_ERROR,
      2,
      connection,
...
void NodeEventHandler::onUserAccountUpdated(const RtcConnection& connection, uid_t remoteUid, const char* userAccount)
{
  FUNC_TRACE;
  node_async_call::async_call([this, connection, remoteUid, userAccount] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
      RTC_EVENT_USER_ACCOUNT_UPDATED,
      3,
      connection,
      napi_create_uint32_(isolate, remoteUid),
