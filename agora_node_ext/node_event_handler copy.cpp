...
void NodeEventHandler::onConnectionStateChanged(
  ...
  node_async_call::async_call([this, _connection, state, reason] {
    Isolate *isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_CONNECTION_STATE_CHANED, 3,
...
void NodeEventHandler::onRemoteVideoStateChanged(
    const RtcConnection &connection, uid_t uid, REMOTE_VIDEO_STATE state,
  ...
  node_async_call::async_call([this, _connection, uid, state, reason] {
    ...
    this->sendJSWithConnection(RTC_EVENT_REMOTE_VIDEO_STATE_CHANGED, 4,
                               _connection, napi_create_uid_(isolate, uid),
...
void NodeEventHandler::onRemoteAudioStateChanged(
    const RtcConnection &connection, uid_t uid, REMOTE_AUDIO_STATE state,
  ...
  node_async_call::async_call([this, _connection, uid, state, reason] {
    Isolate *isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(RTC_EVENT_REMOTE_AUDIO_STATE_CHANGED, 4,
                               _connection, napi_create_uid_(isolate, uid),
                               napi_create_int32_(isolate, state),
                               napi_create_int32_(isolate, reason));
  });
...
void NodeEventHandler::onLocalVideoStateChanged(
    const RtcConnection &connection, LOCAL_VIDEO_STREAM_STATE localVideoState,
