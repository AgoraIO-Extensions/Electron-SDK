...
void NodeEventHandler::onAudioQuality(const RtcConnection& connection, uid_t remoteUid, int quality, unsigned short delay, unsigned short lost)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, quality, delay, lost] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onNetworkQuality(const RtcConnection& connection, uid_t remoteUid, int txQuality, int rxQuality)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, txQuality, rxQuality] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onFirstRemoteVideoDecoded(const RtcConnection& connection, uid_t remoteUid, int width, int height, int elapsed)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, width, height, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onVideoSizeChanged(const RtcConnection& connection, uid_t uid, int width, int height, int rotation)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, uid, width, height, rotation] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onStreamMessage(const RtcConnection& connection, uid_t remoteUid, int streamId, const char* data, size_t length, uint64_t sentTs)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, streamId, data, length, sentTs] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onStreamMessageError(const RtcConnection& connection, uid_t remoteUid, int streamId, int code, int missed, int cached)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, streamId, code, missed, cached] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
...
void NodeEventHandler::onRemoteAudioStateChanged(const RtcConnection& connection, uid_t remoteUid, REMOTE_AUDIO_STATE state, REMOTE_AUDIO_STATE_REASON reason, int elapsed)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, state, reason, elapsed] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
  ...
  unsigned short rxKBitRate)
{
  FUNC_TRACE;
  CustomRtcConnection _connection(connection);
  node_async_call::async_call([this, _connection, remoteUid, delay, lost, rxKBitRate] {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    this->sendJSWithConnection(
