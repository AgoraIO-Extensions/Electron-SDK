...
bool NodeMetadataObserver::onReadyToSendMetadata(
    Metadata &metadata, VIDEO_SOURCE_TYPE source_type) {
  std::lock_guard<std::mutex> lock(queueMutex);
  if (!messageQueue.empty() && messageQueue.size() > 0) {
    ...
    if (cachedMetadata) {
      ...
      if (cachedMetadata->buffer) {
        memcpy(metadata.buffer, cachedMetadata->buffer,
               metadata.size * sizeof(char));
        ...
        node_async_call::async_call([this, _uid, _size, _buffer, _timeStampMs] {
          ...
          if (!messageSendCallback.IsEmpty() && !js_this.IsEmpty()) {
            messageSendCallback.Get(isolate)->Call(
                context, js_this.Get(isolate), 1, arg);
          }
        });
