 ...
 */
#define media_player_napi_set_int_result(args, result)                         \
...
NodeMediaPlayer::NodeMediaPlayer(Isolate *_isolate,
                                 IMediaPlayerSourceWraper *mediaPlayer)
    : isolate(_isolate), mMediaPlayer(mediaPlayer) {
  LOG_F(INFO, "NodeMediaPlayer::NodeMediaPlayer");
  nodeMediaPlayerObserver = new NodeMediaPlayerObserver();
  mMediaPlayer->m_mediaPlayerSource->registerPlayerSourceObserver(
      nodeMediaPlayerObserver);
}

NodeMediaPlayer::~NodeMediaPlayer() {
  mMediaPlayer->m_mediaPlayerSource->unregisterPlayerSourceObserver(
      nodeMediaPlayerObserver);
...
Local<Object> NodeMediaPlayer::Init(Isolate *isolate,
  ...
  IMediaPlayerSourceWraper *pMediaPlayerSourceWrper =
      new IMediaPlayerSourceWraper();
  pMediaPlayerSourceWrper->m_mediaPlayerSource = mediaPlayer;
  Local<v8::External> argMediaPlayer = Local<v8::External>::New(
      isolate, v8::External::New(isolate, pMediaPlayerSourceWrper));
  Local<v8::Value> argv[1] = {argMediaPlayer};
  Local<Object> jsmediaPlayer =
...
void NodeMediaPlayer::createInstance(const FunctionCallbackInfo<Value> &args) {
  ...
  IMediaPlayerSourceWraper *pMediaPlayer =
      static_cast<IMediaPlayerSourceWraper *>(argMediaPlayer->Value());
...
NAPI_API_DEFINE_MEDIA_PLAYER(NodeMediaPlayer, open) {
  ...
  do {
    ...
    CHECK_NAPI_STATUS(mediaPlayer, status);
    result =
        mediaPlayer->mMediaPlayer->m_mediaPlayerSource->open(url, position);
  } while (false);
...
NAPI_API_DEFINE_MEDIA_PLAYER(NodeMediaPlayer, getStreamInfo) {
  ...
  do {
    ...
    agora::media::base::PlayerStreamInfo *streamInfo =
        new agora::media::base::PlayerStreamInfo();
    result = mediaPlayer->mMediaPlayer->m_mediaPlayerSource->getStreamInfo(
        index, streamInfo);
    Local<Object> obj = Object::New(isolate);
    obj->Set(context, napi_create_string_(isolate, "streamIndex"),
             napi_create_int32_(isolate, streamInfo->streamIndex));
    obj->Set(context, napi_create_string_(isolate, "streamType"),
             napi_create_int32_(isolate, (int)(streamInfo->streamType)));
    obj->Set(context, napi_create_string_(isolate, "codecName"),
             napi_create_string_(isolate,
                                 std::string(streamInfo->codecName).c_str()));
    obj->Set(context, napi_create_string_(isolate, "language"),
             napi_create_string_(isolate,
                                 std::string(streamInfo->language).c_str()));
    obj->Set(context, napi_create_string_(isolate, "videoFrameRate"),
             napi_create_int32_(isolate, streamInfo->videoFrameRate));
    obj->Set(context, napi_create_string_(isolate, "videoBitRate"),
             napi_create_int32_(isolate, (int)(streamInfo->videoBitRate)));
    obj->Set(context, napi_create_string_(isolate, "videoWidth"),
             napi_create_int32_(isolate, streamInfo->videoWidth));
    obj->Set(context, napi_create_string_(isolate, "videoHeight"),
             napi_create_int32_(isolate, (int)(streamInfo->videoHeight)));
    obj->Set(context, napi_create_string_(isolate, "videoRotation"),
             napi_create_int32_(isolate, streamInfo->videoRotation));
    obj->Set(context, napi_create_string_(isolate, "audioSampleRate"),
             napi_create_int32_(isolate, (int)(streamInfo->audioSampleRate)));
    obj->Set(context, napi_create_string_(isolate, "audioChannels"),
             napi_create_int32_(isolate, streamInfo->audioChannels));
    obj->Set(context, napi_create_string_(isolate, "duration"),
             napi_create_int32_(isolate, (int)(streamInfo->duration)));
...
NAPI_API_DEFINE_MEDIA_PLAYER(NodeMediaPlayer, onEvent) {
  do {
    ...
    mediaPlayer->nodeMediaPlayerObserver->addEventHandler((char *)eventName,
                                                          persistObj, persist);
  } while (false);
  // LOG_LEAVE;
}
