...
void NodeVideoFrameTransporter::subscribe(NodeRenderType type,
                                          agora::rtc::uid_t uid,
...
void NodeVideoFrameTransporter::unsubscribe(NodeRenderType type,
                                            agora::rtc::uid_t uid,
                                            std::string channelId,
                                            int deviceId) {
  std::lock_guard<std::mutex> lck(m_lock);
...
VideoFrameInfo &NodeVideoFrameTransporter::getVideoFrameInfo(
  ...
  if (type == NodeRenderType::NODE_RENDER_TYPE_LOCAL) {
    if (m_localVideoFrames.find(deviceId) == m_localVideoFrames.end()) {
      m_localVideoFrames[deviceId] =
          VideoFrameInfo(NODE_RENDER_TYPE_LOCAL, deviceId);
    }
    return m_localVideoFrames[deviceId];
  } else if (type == NODE_RENDER_TYPE_TRANSCODED) {
    if (!m_transcodedVideoFrame.get()) {
      m_transcodedVideoFrame.reset(
          new VideoFrameInfo(NODE_RENDER_TYPE_TRANSCODED));
    }
    ...
    if (m_screenCaptureFrames.find(deviceId) == m_screenCaptureFrames.end()) {
      m_screenCaptureFrames[deviceId] =
          VideoFrameInfo(NODE_RENDER_TYPE_VIDEO_SOURCE, deviceId);
    }
...
int NodeVideoFrameTransporter::deliverVideoSourceFrame(const char *payload,
  ...
  // image_header_type *hdr = (image_header_type*)(payload +
  // sizeof(image_frame_info)); unsigned int uv_len = (info->stride / 2) *
  // (info->height / 2); char* y = (char*)(hdr + 1); char* u = y + uv_len * 4;
  // char* v = u + uv_len;
  // std::lock_guard<std::mutex> lck(m_lock);
  // VideoFrameInfo& videoInfo =
  // getVideoFrameInfo(NODE_RENDER_TYPE_VIDEO_SOURCE, 0, 0); int destWidth =
  // videoInfo.m_destWidth ? videoInfo.m_destWidth : info->width; int destHeight
  // = videoInfo.m_destHeight ? videoInfo.m_destHeight : info->height; size_t
  // imageSize = sizeof(image_header_type) + destWidth * destHeight * 3 / 2;
  ...
  //     (uint8*)desty, destWidth, (uint8*)destu, destWidth / 2, (uint8*)destv,
  //     destWidth / 2, destWidth, destHeight, kFilterNone);
...
void NodeVideoFrameTransporter::copyFrame(
    const agora::media::IVideoFrameObserver::VideoFrame &videoFrame,
    VideoFrameInfo &info, int dest_stride, int src_stride, int width,
    int height) {
  int width2 = dest_stride / 2, heigh2 = height / 2;
  int strideY = videoFrame.yStride;
  int strideU = videoFrame.uStride;
  int strideV = videoFrame.vStride;
  ...
  const unsigned char *planeY = (const unsigned char *)videoFrame.yBuffer;
  const unsigned char *planeU = (const unsigned char *)videoFrame.uBuffer;
  const unsigned char *planeV = (const unsigned char *)videoFrame.vBuffer;
  ...
  while (!m_stopFlag) {
    {
      ...
      // if (m_videoSourceVideoFrame.get() && m_videoSourceVideoFrame->m_count >
      // MAX_MISS_COUNT)
