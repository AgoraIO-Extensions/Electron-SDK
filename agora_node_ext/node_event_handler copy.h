...
#define RTC_EVENT_FIRST_LOCAL_VIDEO_FRAME_PUBLISH "firstLocalVideoFramePublished"
    // RTC_EVENT_VIDEO_FRAME_SIZE_CHANGED->RTC_EVENT_VIDEO_SOURCE_FRAME_SIZE_CHANGED
...
#define RTC_EVENT_API_ERROR "apierror"
    ...
    public:
      ...
      virtual void onFacePositionChanged(int imageWidth, int imageHeight,
        const Rectangle* vecRectangle, const int* vecDistance, int numFaces) override;
#endif
      ...
      // para changed
      virtual void onExtensionEvent(const char* provider_name, const char* ext_name, const char* key, const char* json_value) override;
      // new
      virtual void onExtensionStarted(const char* provider_name, const char* ext_name) override;
      // new
      virtual void onExtensionStopped(const char* provider_name, const char* ext_name) override;
      virtual void onExtensionErrored(const char* provider_name, const char* ext_name, int error, const char* msg) override;
      ...
      virtual void onAudioQuality(const RtcConnection& connection, uid_t remoteUid, int quality, unsigned short delay, unsigned short lost) override;
      virtual void onAudioVolumeIndication(const RtcConnection& connection, const AudioVolumeInfo* sperkers, unsigned int speakerNumber, int totalVolume) override;
      virtual void onLeaveChannel(const RtcConnection& connection, const RtcStats& stats) override;
      ...
      virtual void onFirstLocalVideoFrame(const RtcConnection& connection, int width, int height, int elapsed) override;
      virtual void onFirstLocalVideoFramePublished(const RtcConnection& connection, int elapsed) override;
      virtual void onVideoSourceFrameSizeChanged(const RtcConnection& connection, VIDEO_SOURCE_TYPE sourceType, int width, int height) override;
      ...
      virtual void onVideoSizeChanged(const RtcConnection& connection, uid_t uid, int width, int height, int rotation) override;
      // [para] localVideoState->state   error->errorCode
      ...
      virtual void onLocalVideoStateChanged(const RtcConnection& connection, LOCAL_VIDEO_STREAM_STATE state, LOCAL_VIDEO_STREAM_ERROR errorCode) override;
      // [para] uid->remoteUid
      ...
      virtual void onUserJoined(const RtcConnection& connection, uid_t remoteUid, int elapsed) override;
      virtual void onUserOffline(const RtcConnection& connection, uid_t remoteUid, USER_OFFLINE_REASON_TYPE reason) override;
      ...
      virtual void onUserEnableLocalVideo(const RtcConnection& connection, uid_t remoteUid, bool enabled) override;
      ...
      virtual void onConnectionInterrupted(const RtcConnection& connection) override;
      ...
      virtual void onClientRoleChanged(const RtcConnection& connection, CLIENT_ROLE_TYPE oldRole, CLIENT_ROLE_TYPE newRole) override;
      ...
      virtual void onRemoteVideoTransportStats(const RtcConnection& connection, uid_t remoteUid, unsigned short delay, unsigned short lost,
        unsigned short rxKBitRate) override;
      virtual void onConnectionStateChanged(const RtcConnection& connection, CONNECTION_STATE_TYPE state, CONNECTION_CHANGED_REASON_TYPE reason) override;
      ...
      virtual void onUserAccountUpdated(const RtcConnection& connection, uid_t remoteUid, const char* userAccount) override;

    private:
      std::unordered_map<std::string, NodeEventCallback*> m_callbacks;
      NodeRtcEngine* m_engine;

      void onAudioVolumeIndication_node(AudioVolumeInfo* speakers, unsigned int speakerNumber, int totalVolume);
