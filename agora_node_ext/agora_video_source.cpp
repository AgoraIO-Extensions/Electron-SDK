...
class AgoraVideoSourceSink : public AgoraVideoSource, public AgoraIpcListener {
 public:
  ...
  virtual bool initialize(IAgoraVideoSourceEventHandler *eventHandler,
                          ...
                          const char *groupId, const char *bundleId) override;
  ...
  setVideoSourceChannelProfile(agora::rtc::CHANNEL_PROFILE_TYPE profile,
                               const char *permissionKey) override;
  virtual node_error
  ...
  virtual node_error
  setScreenCaptureContentHint(VideoContentHint contentHint) override;
  ...
  virtual node_error
  enableEncryption(bool enable, EncryptionConfig encryptionConfig) override;
  ...
  virtual node_error
  setScreenCaptureScenario(agora::rtc::SCREEN_SCENARIO_TYPE type) override;
...
node_error AgoraVideoSourceSink::release() {
  LOG_ENTER;
  if (m_initialized) {
    LOG_INFO("AgoraVideoSourceSink:%s release m_initialized %d", __FUNCTION__,
             m_initialized);
    clear();
  }
...
bool AgoraVideoSourceSink::initialize(
    ...
    unsigned int areaCode, const char *groupId, const char *bundleId) {
  LOG_ENTER;

  LOG_INFO("VideoSource: AgoraVideoSourceSink %s m_initialized %d",
           __FUNCTION__, m_initialized);
  if (m_initialized) {
    LOG_INFO("VideoSource: AgoraVideoSourceSink %s already initialized",
             __FUNCTION__);
    return true;
  }

  LOG_INFO("VideoSource: AgoraVideoSourceSink %s appid %s", __FUNCTION__,
           appid);

  if (!appid) {
    LOG_ERROR("VideoSource: AgoraVideoSourceSink %s appid not get",
              __FUNCTION__);
    return false;
  }
  clear();
  ...
  if (UuidToStringA(&uuid, &struuid) != RPC_S_OK) {
    LOG_ERROR("VideoSource: AgoraVideoSourceSink %s UuidToStringA",
              __FUNCTION__);
    return false;
  }
  ...
  do {
    ...
    if (!m_ipcMsg.get()) {
      LOG_ERROR("VideoSource: AgoraVideoSourceSink %s m_ipcMsg not get",
                __FUNCTION__);
      break;
    }
    ...
    if (!m_ipcMsg->initialize(m_peerId)) {
      LOG_ERROR(
          "VideoSource: AgoraVideoSourceSink %s m_ipcMsg initialize m_peerId "
          "%s fail",
          __FUNCTION__, m_peerId.c_str());
      break;
    }

    if (!m_ipcMsg->listen()) {
      LOG_ERROR(
          "VideoSource: AgoraVideoSourceSink %s m_ipcMsg initialize not listen",
          __FUNCTION__);
      break;
    }
    ...
    if (!INodeProcess::getCurrentModuleFileName(targetPath)) {
      LOG_ERROR("VideoSource: AgoraVideoSourceSink %s getCurrentModuleFileName "
                "targetPath %s",
                __FUNCTION__, targetPath.c_str());
      break;
    }

    size_t pos = targetPath.find_last_of("\\/");
    if (pos == targetPath.npos) {
      LOG_ERROR("VideoSource: AgoraVideoSourceSink %s targetPath.pos not equal",
                __FUNCTION__);
      break;
    }
    ...
    const char *params[] = {cmdname.c_str(),  idparam.c_str(),
                            pidparam.c_str(), appidparam.c_str(),
                            acparam.c_str(),  nullptr};
    m_sourceNodeProcess.reset(
        INodeProcess::CreateNodeProcess(path.c_str(), params));
    if (!m_sourceNodeProcess.get()) {
      LOG_ERROR(
          "VideoSource: AgoraVideoSourceSink %s m_sourceNodeProcess not get",
          __FUNCTION__);
      break;
    }
    ...
    if (result != NodeEvent::WAIT_RESULT_SET) {
      LOG_ERROR("VideoSource: AgoraVideoSourceSink %s WAIT_RESULT_SET",
                __FUNCTION__);
      break;
    }
...
node_error AgoraVideoSourceSink::startPreview() {
  LOG_ENTER;
  return m_ipcMsg->sendMessage(AGORA_IPC_START_VS_PREVIEW, nullptr, 0)
      ? node_ok
      : node_generic_error;
}
...
void AgoraVideoSourceSink::onStartPreviewComplete() {
  ...
  if (!m_ipcReceiver->initialize(m_peerId + DATA_IPC_NAME,
                                 std::bind(&AgoraVideoSourceSink::deliverFrame,
...
node_error AgoraVideoSourceSink::join(const char *token, const char *cname,
  ...
  if (m_initialized) {
    ...
    if (token) strncpy(cmd->token, token, MAX_TOKEN_LEN);
    if (cname) { strncpy(cmd->cname, cname, MAX_CNAME_LEN); }
    if (chan_info) strncpy(cmd->chan_info, chan_info, MAX_CHAN_INFO);
    cmd->uid = uid;
...
node_error AgoraVideoSourceSink::setLocalAccessPoint(
  ...
  LOG_ENTER;
  if (!m_ipcMsg) { return node_generic_error; }
...
node_error AgoraVideoSourceSink::setVideoSourceChannelProfile(
    agora::rtc::CHANNEL_PROFILE_TYPE profile, const char *permissionKey) {
  LOG_ENTER;
  if (m_initialized) {
...
#endif
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_CHANNEL_PROFILE,
                                 (char *) &profile, sizeof(profile))
        ? node_ok
        : node_generic_error;
  }
  return node_status_error;
...
node_error
AgoraVideoSourceSink::setCloudProxy(agora::rtc::CLOUD_PROXY_TYPE type) {
  LOG_ENTER;
  if (m_initialized) {
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_CLOUD_PROXY, (char *) &type,
                                 sizeof(type))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::muteLocalVideoStream(bool mute) {
  LOG_ENTER;
  if (m_initialized) {
    return m_ipcMsg->sendMessage(AGORA_IPC_MUTE_LOCAL_VIDEO_STREAM,
...
node_error AgoraVideoSourceSink::setScreenCaptureScenario(
    agora::rtc::SCREEN_SCENARIO_TYPE type) {
  LOG_ENTER;
  if (m_initialized) {
...
void AgoraVideoSourceSink::onMessage(unsigned int msg, char *payload,
  ...
  if (msg == AGORA_IPC_JOIN_SUCCESS) {
    ...
    if (m_eventHandler) { m_eventHandler->onVideoSourceLeaveChannel(); }
  } else if (msg == AGORA_IPC_RENEW_TOKEN) {
    if (m_eventHandler) { m_eventHandler->onVideoSourceRequestNewToken(); }
...
node_error
AgoraVideoSourceSink::captureScreen(agora::rtc::IRtcEngine::WindowIDType id,
                                    int captureFreq, agora::rtc::Rect *rect,
                                    int bitrate) {
  LOG_ENTER;
  if (m_initialized) {
  ...
  LOG_ERROR("AgoraVideoSourceSink: captureScreen error WindowIDType: %d "
            "captureFreq:%d  bitrate:%d",
            __FUNCTION__, id, captureFreq, bitrate);
  return node_status_error;
}
...
node_error AgoraVideoSourceSink::startScreenCaptureByScreen(
    ScreenIDType screenId, const agora::rtc::Rectangle &regionRect,
    const agora::rtc::ScreenCaptureParameters &captureParams,
    const std::vector<agora::rtc::IRtcEngine::WindowIDType> &excludeWindows) {
  LOG_ENTER;
  if (m_initialized) {
    ...
    return m_ipcMsg->sendMessage(AGORA_IPC_START_CAPTURE_BY_DISPLAY,
                                 (char *) &cmd, sizeof(cmd))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::startScreenCaptureByDisplayId(
    DisplayInfo displayId, const Rectangle &regionRect,
    const agora::rtc::ScreenCaptureParameters &captureParams,
    const std::vector<agora::rtc::IRtcEngine::WindowIDType> &excludeWindows) {
  if (m_initialized) {
    ...
    return m_ipcMsg->sendMessage(AGORA_IPC_START_SCREEN_CAPTURE_BY_DISPLAY_ID,
                                 (char *) &cmd, sizeof(cmd))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::startScreenCaptureByWindow(
    agora::rtc::IRtcEngine::WindowIDType windowId, const Rectangle &regionRect,
    const agora::rtc::ScreenCaptureParameters &captureParams) {
  LOG_ENTER;
  if (m_initialized) {
    ...
    cmd.regionRect = regionRect;
    cmd.captureParams = captureParams;
    return m_ipcMsg->sendMessage(AGORA_IPC_START_CAPTURE_BY_WINDOW_ID,
                                 (char *) &cmd, sizeof(cmd))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::updateScreenCaptureParameters(
    const agora::rtc::ScreenCaptureParameters &captureParams,
    const std::vector<agora::rtc::IRtcEngine::WindowIDType> &excludeWindows) {
  LOG_ENTER;
  if (m_initialized) {
    ...
    return m_ipcMsg->sendMessage(AGORA_IPC_UPDATE_SCREEN_CAPTURE_PARAMS,
                                 (char *) &cmd, sizeof(cmd))
        ? node_ok
        : node_generic_error;
  }
...
node_error
AgoraVideoSourceSink::enableLoopbackRecording(bool enabled,
  ...
  if (m_initialized) {
    ...
    return m_ipcMsg->sendMessage(AGORA_IPC_ENABLE_LOOPBACK_RECORDING,
                                 (char *) &cmd, sizeof(cmd))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::setEncryptionMode(const char *encryptionMode) {
  LOG_ENTER;
  if (m_initialized) {
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_ENCRYPTION_MODE,
                                 (char *) encryptionMode, sizeof(const char *))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::setEncryptionSecret(const char *secret) {
  LOG_ENTER;
  if (m_initialized) {
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_ENCRYPTION_SECRET,
                                 (char *) secret, sizeof(const char *))
        ? node_ok
        : node_generic_error;
  }
...
node_error
AgoraVideoSourceSink::enableEncryption(bool enable,
  ...
  if (m_initialized) {
    ...
    if (encryptionConfig.encryptionKey) {
      strncpy(cmd.encryptionKey, encryptionConfig.encryptionKey,
              MAX_DEVICE_ID_LENGTH);
    }
    return m_ipcMsg->sendMessage(AGORA_IPC_ENABLE_ENCRYPTION, (char *) &cmd,
                                 sizeof(cmd))
        ? node_ok
        : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::setProcessDpiAwareness() {
  LOG_ENTER;
  if (m_initialized) {
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_PROCESS_DPI_AWARE_NESS, nullptr,
                                 0)
        ? node_ok
        : node_generic_error;
  }
