...
class AgoraVideoSourceSink : public AgoraVideoSource, public AgoraIpcListener {
 public:
  ...
  virtual node_error setScreenCaptureContentHint(
      VideoContentHint contentHint) override;
  ...
  virtual node_error enableEncryption(
      bool enable,
      EncryptionConfig encryptionConfig) override;
...
bool AgoraVideoSourceSink::initialize(
    IAgoraVideoSourceEventHandler* eventHandler,
    const char* appid,
    unsigned int areaCode,
    const char* groupId,
    const char* bundleId) {
  ...
  do {
    ...
    if (!INodeProcess::getCurrentModuleFileName(targetPath)) {
      LOG_ERROR(
          "VideoSource: AgoraVideoSourceSink %s getCurrentModuleFileName "
          "targetPath %s",
...
#endif
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_CHANNEL_PROFILE, (char*)&profile,
                                 sizeof(profile))
               ? node_ok
               : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::captureScreen(
    agora::rtc::IRtcEngine::WindowIDType id,
    int captureFreq,
    agora::rtc::Rect* rect,
    int bitrate) {
  LOG_ENTER;
  ...
  LOG_ERROR(
      "AgoraVideoSourceSink: captureScreen error WindowIDType: %d "
...
node_error AgoraVideoSourceSink::enableLoopbackRecording(
    bool enabled,
...
node_error AgoraVideoSourceSink::setEncryptionSecret(const char* secret) {
  LOG_ENTER;
  if (m_initialized) {
    return m_ipcMsg->sendMessage(AGORA_IPC_SET_ENCRYPTION_SECRET, (char*)secret,
                                 sizeof(const char*))
               ? node_ok
               : node_generic_error;
  }
...
node_error AgoraVideoSourceSink::enableEncryption(
    bool enable,
