...
#define RTC_EVENT_REQUEST_AUDIO_FILE_INFO "requestAudioFileInfo"
#define RTC_EVENT_SNAPSHOT_TAKEN "snapshotTaken"
#define RTC_EVENT_AUDIO_DEVICE_TEST_VOLUME_INDICATION                          \
  "audioDeviceTestVolumeIndication"

/* meeting */
#define RTC_EVENT_VIDEO_SOURCE_SCREEN_CAPTURE_INFO_UPDATED                     \
...
class NodeEventHandler : public IRtcEngineEventHandler,
 ...
 public:
  ...
  virtual void onRejoinChannelSuccess(const char *channel, uid_t uid,
                                      int elapsed) override;
  ...
  virtual void onFirstRemoteVideoDecoded(uid_t uid, int width, int height,
                                         int elapsed) override;
  virtual void onVideoSizeChanged(uid_t uid, int width, int height,
  ...
  virtual void onStreamMessage(uid_t uid, int streamId, const char *data,
                               size_t length) override;
  virtual void onStreamMessageError(uid_t uid, int streamId, int code,
  ...
  virtual void onStreamInjectedStatus(const char *url, uid_t uid,
                                      int status) override;
  virtual void
  ...
  virtual void
  onConnectionStateChanged(CONNECTION_STATE_TYPE state,
  ...
  virtual void
  onVideoSourceLocalAudioStats(const LocalAudioStats &stats) override;
  virtual void onVideoSourceVideoSizeChanged(uid_t uid, int width, int height,
                                             int rotation) override;
  virtual void
  onVideoSourceLocalVideoStats(const LocalVideoStats &stats) override;
  ...
  virtual void
  onLocalVideoStateChanged(LOCAL_VIDEO_STREAM_STATE localVideoState,
                           LOCAL_VIDEO_STREAM_ERROR error) override;

  // 2.9.0
  virtual void onLocalAudioStats(const LocalAudioStats &stats) override;
  virtual void
  onLocalAudioStateChanged(LOCAL_AUDIO_STREAM_STATE state,
                           LOCAL_AUDIO_STREAM_ERROR error) override;
  virtual void onRemoteAudioStateChanged(uid_t uid, REMOTE_AUDIO_STATE state,
                                         REMOTE_AUDIO_STATE_REASON reason,
                                         int elapsed) override;
  virtual void
  onChannelMediaRelayStateChanged(CHANNEL_MEDIA_RELAY_STATE state,
                                  CHANNEL_MEDIA_RELAY_ERROR code) override;
  virtual void
  onChannelMediaRelayEvent(CHANNEL_MEDIA_RELAY_EVENT code) override;
  ...
  // 3.3.0
  virtual void onUploadLogResult(const char *requestId, bool success,
  ...
  // 3.5.1
  virtual void onRequestAudioFileInfo(const agora::rtc::AudioFileInfo &info,
  ...
  // 3.5.2
  virtual void onSnapshotTaken(const char *channel, uid_t uid,
                               const char *filePath, int width, int height,
                               int errCode) override;
  // 3.6.0.2
  virtual void
  onAudioDeviceTestVolumeIndication(AudioDeviceTestVolumeType volumeType,
                                    int volume) override;
 ...
 private:
  ...
  void onAudioQuality_node(uid_t uid, int quality, unsigned short delay,
                           unsigned short lost);
  ...
  void onAudioDeviceStateChanged_node(const char *deviceId, int deviceType,
                                      int deviceState);
  ...
  void onVideoDeviceStateChanged_node(const char *deviceId, int deviceType,
                                      int deviceState);
  ...
  void onFirstRemoteVideoDecoded_node(uid_t uid, int width, int height,
                                      int elapsed);
  ...
  void onRemoteVideoStateChanged_node(uid_t uid, REMOTE_VIDEO_STATE state,
                                      REMOTE_VIDEO_STATE_REASON reason,
                                      int elapsed);
  void onFirstRemoteVideoFrame_node(uid_t uid, int width, int height,
                                    int elapsed);
  ...
  void onStreamMessage_node(uid_t uid, int streamId, const char *data,
                            size_t length);
  void onStreamMessageError_node(uid_t uid, int streamId, int code, int missed,
                                 int cached);
  ...
  void onAudioDeviceVolumeChanged_node(MEDIA_DEVICE_TYPE deviceType, int volume,
                                       bool muted);
  ...
  void onRemoteAudioStateChanged_node(uid_t uid, REMOTE_AUDIO_STATE state,
                                      REMOTE_AUDIO_STATE_REASON reason,
                                      int elapsed);
