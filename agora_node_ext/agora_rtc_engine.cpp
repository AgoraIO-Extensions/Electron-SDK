...
void NodeRtcEngine::Init(Local<Object> &module) {
  ...
  PROPERTY_METHOD_DEFINE(setVideoEncoderConfigurationEx)
...
NAPI_API_DEFINE(NodeRtcEngine, enableLoopbackRecordingEx) {
  ...
  do {
    ...
    status = napi_get_value_object_(isolate, args[1], connectionObj);
    decodeRtcConnection(connection, channelId, status, args, connectionObj);
    CHECK_NAPI_STATUS(pEngine, status);
...
NAPI_API_DEFINE(NodeRtcEngine, setVideoEncoderConfiguration) {
  ...
  do {
    ...
    VideoEncoderConfiguration config;
    decodeVideoEncoderConfiguration(config, status, args, obj);
    CHECK_NAPI_STATUS(pEngine, status);
...
NAPI_API_DEFINE(NodeRtcEngine, setVideoEncoderConfigurationEx) {
  LOG_ENTER;
  int result = -1;
  do {
    napi_status status = napi_ok;
    ...
    result = engineEx->setVideoEncoderConfigurationEx(config, connection);
  } while (false);
  napi_set_int_result(args, result);
  LOG_LEAVE;
}
...
NAPI_API_DEFINE(NodeRtcEngine, leaveChannelEx) {
  ...
  do {
    ...
    Local<Object> connectionObj;
    status = napi_get_value_object_(isolate, args[0], connectionObj);
    decodeRtcConnection(rtcConnection, channelId, status, args, connectionObj);
    CHECK_NAPI_STATUS(pEngine, status);
...
NAPI_API_DEFINE(NodeRtcEngine, joinChannelEx) {
  ...
  do {
    ...
    std::string tokenStr;
    decodeChannelMediaOptions(channelMediaOptions, tokenStr, args, optionObj);
