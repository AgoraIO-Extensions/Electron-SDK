    ...
    namespace rtc {
        ...
        void NodeRtcEngine::Init(Local<Object>& module)
        {
            ...
            BEGIN_PROPERTY_DEFINE(NodeRtcEngine, createInstance, 5)
                ...
                PROPERTY_METHOD_DEFINE(registerRGBAVideoFrameReceiver);
                PROPERTY_METHOD_DEFINE(unregisterRGBAVidioFrameReceiver);
            EN_PROPERTY_DEFINE()
...
#define NODE_SET_OBJ_WINDOWINFO_DATA(isolate, obj, name, info) \
    { \
        ...
        NAPI_API_DEFINE(NodeRtcEngine, registerRGBAVideoFrameReceiver)
        {
            LOG_ENTER;
            int result = -1;
            do {
                ...
                if (!args[0]->IsFunction()) {
                    LOG_ERROR("registerRGBAVideoFrameReceiver: Function expected");
                    break;
                }

                Local<Function> callback = args[0].As<Function>();
                if (callback.IsEmpty()) {
                    LOG_ERROR("registerRGBAVideoFrameReceiver: Function is empty.");
                    break;
                }
                ...
                NodeVideoFrameTransporter* pTransporter = getNodeVideoFrameTransporter();
                if(pTransporter) {
                    pTransporter->AddRGBAVideoFrameCallback(persist);
                    result = 0;
                }
            } while (false);
            napi_set_int_result(args, result);
        ...
        NAPI_API_DEFINE(NodeRtcEngine, unregisterRGBAVidioFrameReceiver)
        {
            LOG_ENTER;
            int result = false;
            do {
                ...
                NodeVideoFrameTransporter* pTransporter = getNodeVideoFrameTransporter();
                if(pTransporter) {
                    pTransporter->RemoveRGBAVideoFrameCallback();
                    result = 0;
                }
            } while (false);
            napi_set_int_result(args, result);
