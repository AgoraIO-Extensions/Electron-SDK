    ...
    { \
        ...
        NAPI_API_DEFINE(NodeRtcEngine, getScreenDisplaysInfo)
        {
            ...
            do {
                ...
                for (unsigned int i = 0; i < allDisplays.size(); ++i) {
                    ...
                    NODE_SET_OBJ_PROP_UINT32(isolate, displayIdObj, "x", displayInfo.x);
                    NODE_SET_OBJ_PROP_UINT32(isolate, displayIdObj, "y", displayInfo.y);
        ...
        NAPI_API_DEFINE(NodeRtcEngine, getScreenCaptureSources)
        {
          ...
          do {
            ...
            status = napi_get_object_property_int32_(isolate, thumbSizeObj, "width", thumbWidth);
            CHECK_NAPI_STATUS(pEngine, status);
            status = napi_get_object_property_int32_(isolate, thumbSizeObj, "height", thumbHeight);
            CHECK_NAPI_STATUS(pEngine, status)
            ...
            status = napi_get_object_property_int32_(isolate, iconSizeSizeObj, "width", iconWidth);
            CHECK_NAPI_STATUS(pEngine, status);
            status = napi_get_object_property_int32_(isolate, iconSizeSizeObj, "height", iconHeight);
            CHECK_NAPI_STATUS(pEngine, status);
#if defined(_WIN32)
            ...
            auto list = pEngine->m_engine->getScreenCaptureSources(thumbSize, iconSize, includeScreen);
...
#else
              auto sourceId = NSNumberToLong(info.sourceId);
#endif
              
              std::string sourceName(info.sourceName == nullptr ? "" : info.sourceName);
              std::string processPath(info.processPath == nullptr ? "" : info.processPath);
              std::string sourceTitle(info.sourceTitle == nullptr ? "" : info.sourceTitle);
              bool primaryMonitor = info.primaryMonitor;
              
              NODE_SET_OBJ_PROP_UINT32(isolate, obj, "type", type);
              NODE_SET_OBJ_PROP_UINT32(isolate, obj, "sourceId", (unsigned long)sourceId);
              NODE_SET_OBJ_PROP_String(isolate, obj, "sourceName", sourceName.c_str());
              NODE_SET_OBJ_PROP_String(isolate, obj, "processPath", processPath.c_str());
              NODE_SET_OBJ_PROP_String(isolate, obj, "sourceTitle", sourceTitle.c_str());
...
#if defined(_WIN32)
                  ConvertRGBToBMP(const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(info.iconImage.buffer)), iconImageInfo, info.iconImage.width, info.iconImage.height);
                  NODE_SET_OBJ_WINDOWINFO_DATA(isolate, iconImageObj, "buffer", iconImageInfo);
                  free(iconImageInfo.buffer);
#else
                  auto buffer = reinterpret_cast<const unsigned char*>(info.iconImage.buffer);
                  iconImageInfo.buffer = const_cast<unsigned char*>(buffer);
                  iconImageInfo.length = info.iconImage.length;
                  NODE_SET_OBJ_WINDOWINFO_DATA(isolate, iconImageObj, "buffer", iconImageInfo);
#endif
                  NODE_SET_OBJ_PROP_UINT32(isolate, iconImageObj, "width", info.iconImage.width);
                  NODE_SET_OBJ_PROP_UINT32(isolate, iconImageObj, "height", info.iconImage.height);
                
                  Local<Value> propName = String::NewFromUtf8(isolate, "iconImage", NewStringType::kInternalized).ToLocalChecked();
...
#if defined(_WIN32)
                  ConvertRGBToBMP(const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(info.thumbImage.buffer)), thumbImageInfo, info.thumbImage.width, info.thumbImage.height);
                  NODE_SET_OBJ_WINDOWINFO_DATA(isolate, thumbImageObj, "buffer", thumbImageInfo);
                  free(thumbImageInfo.buffer);
#else
                  auto buffer = reinterpret_cast<const unsigned char*>(info.thumbImage.buffer);
                  thumbImageInfo.buffer = const_cast<unsigned char*>(buffer);
                  thumbImageInfo.length = info.thumbImage.length;
                    NODE_SET_OBJ_WINDOWINFO_DATA(isolate, thumbImageObj, "buffer", thumbImageInfo);
#endif
                  NODE_SET_OBJ_PROP_UINT32(isolate, thumbImageObj, "width", info.thumbImage.width);
                  NODE_SET_OBJ_PROP_UINT32(isolate, thumbImageObj, "height", info.thumbImage.height);
                  Local<Value> propName = String::NewFromUtf8(isolate, "thumbImage", NewStringType::kInternalized).ToLocalChecked();
