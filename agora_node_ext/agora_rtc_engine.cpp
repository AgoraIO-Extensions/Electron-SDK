    ...
    namespace rtc {
        ...
        NAPI_API_DEFINE(NodeRtcEngine, setLiveTranscoding)
        {
            ...
            do {
                ...
                if (!bgValue->IsNullOrUndefined()) {
                    Local<Object> objBackgroundImage;
                    napi_get_value_object_(isolate, bgValue, objBackgroundImage);
                    
                    status = napi_get_object_property_nodestring_(isolate, objBackgroundImage, "url", bgurl);
                    CHECK_NAPI_STATUS(pEngine, status);
                    bgImage.url = bgurl;

                    status = napi_get_object_property_int32_(isolate, objBackgroundImage, "x", bgImage.x);
                    CHECK_NAPI_STATUS(pEngine, status);

                    status = napi_get_object_property_int32_(isolate, objBackgroundImage, "y", bgImage.y);
                    CHECK_NAPI_STATUS(pEngine, status);

                    status = napi_get_object_property_int32_(isolate, objBackgroundImage, "width", bgImage.width);
                    CHECK_NAPI_STATUS(pEngine, status);
                    
                    status = napi_get_object_property_int32_(isolate, objBackgroundImage, "height", bgImage.height);
                    CHECK_NAPI_STATUS(pEngine, status);
        ...
        NAPI_API_DEFINE(NodeRtcEngine, videoSourceInitialize)
        {
            ...
            do{
                ...
                CHECK_NAPI_STATUS(pEngine, status);
                unsigned int areaCode = AREA_CODE_GLOB;
        ...
        NAPI_API_DEFINE(NodeRtcEngine, initialize)
        {
            ...
            do {
                ...
                CHECK_NAPI_STATUS(pEngine, status);
                int areaCode = -1;
                status = napi_get_value_int32_(args[1], areaCode);
...
#define NODE_SET_OBJ_WINDOWINFO_DATA(isolate, obj, name, info) \
    { \
         ...
         * 3.0.1 Apis
         */
        NAPI_API_DEFINE_WRAPPER_SET_PARAMETER_1(setAudioMixingPitch, int32);
        //NAPI_API_DEFINE_WRAPPER_SET_PARAMETER_2(adjustUserPlaybackSignalVolume, uid_t, int32);

        NAPI_API_DEFINE(NodeRtcEngine, adjustUserPlaybackSignalVolume)
        {
            LOG_ENTER;
            int result = -1;
            do {
                ...
                CHECK_NATIVE_THIS(pEngine);
                unsigned int uid;
                status = napi_get_value_uint32_(args[0], uid);
                CHECK_NAPI_STATUS(pEngine, status);
                int volume;
                status = napi_get_value_int32_(args[1], volume);
                CHECK_NAPI_STATUS(pEngine, status);

                result = pEngine->m_engine->adjustUserPlaybackSignalVolume(uid, volume);
            } while (false);
            napi_set_int_result(args, result);
            LOG_LEAVE;
        }    
