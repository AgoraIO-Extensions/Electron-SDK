...
enum NodeRenderType {
  ...
  NODE_RENDER_TYPE_VIDEO_SOURCE = 3,
  NODE_RENDER_TYPE_TRANSCODED = 4
};
...
public:
  ...
  VideoFrameInfo()
      : m_renderType(NODE_RENDER_TYPE_REMOTE), m_uid(0), m_destWidth(0),
        ...
        m_deviceId(0), m_subscribed(false) {}
  VideoFrameInfo(NodeRenderType type)
      : m_renderType(type), m_uid(0), m_destWidth(0), m_destHeight(0),
        m_needUpdate(false), m_count(0), m_channelId(""), m_deviceId(0),
        m_subscribed(false) {}
  VideoFrameInfo(NodeRenderType type, int deviceId)
      : m_renderType(type), m_uid(0), m_destWidth(0), m_destHeight(0),
        m_needUpdate(false), m_count(0), m_channelId(""), m_deviceId(deviceId),
        m_subscribed(false) {}
  VideoFrameInfo(NodeRenderType type, agora::rtc::uid_t uid,
                 std::string channelId, agora::rtc::conn_id_t connectionId)
      : m_renderType(type), m_uid(uid), m_destWidth(0), m_destHeight(0),
        m_needUpdate(false), m_count(0), m_channelId(channelId), m_deviceId(0),
        m_subscribed(false) {}
  VideoFrameInfo(NodeRenderType type, agora::rtc::uid_t uid,
...
public:
  ...
  int setVideoDimension(NodeRenderType, agora::rtc::uid_t uid,
                        std::string channelId, int deviceId, uint32_t width,
                        uint32_t height);
  ...
  void subscribe(NodeRenderType type, agora::rtc::uid_t uid,
                 std::string channelId, int deviceId);
  void unsubscribe(NodeRenderType type, agora::rtc::uid_t uid,
                   std::string channelId, int deviceId);
...
private:
  ...
  void setupFrameHeader(image_header_type *header, int stride, int width,
                        int height);
  void
  copyFrame(const agora::media::IVideoFrameObserver::VideoFrame &videoFrame,
 ...
 */
#define DECLARE_CLASS static Persistent<Function> constructor
/** Define the class */
#define DEFINE_CLASS(name) Persistent<Function> name::constructor
 ...
 */
// #define PROPERTY_METHOD_DEFINE(name) NODE_SET_PROTOTYPE_METHOD(tpl, #name,
// name);
