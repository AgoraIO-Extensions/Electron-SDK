...
struct AudioPluginFrame {
  int type;
  /** Number of samples in the audio frame: samples = (int)samplesPerCall =
   * (int)(sampleRate &times; sampleInterval)
   */
  int samples;  // number of samples in this frame
  /** Number of bytes per audio sample. For example, each PCM audio sample
  ...
  */
  ...
  /** Audio frame data buffer. The valid data length is: samples &times;
   * channels &times; bytesPerSample
   */
...
class IAVFramePluginCallback {
 public:
  ...
  virtual bool onPluginPlaybackAudioFrameBeforeMixing(
      unsigned int uid,
