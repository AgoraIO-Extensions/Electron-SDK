...
IExternalVideoRender *NodeVideoRenderFactory::createRenderInstance(
...
NodeVideoRender::NodeVideoRender(NodeRenderContext *nrc,
                                 const ExternalVideoRenerContext &context) {
...
int NodeVideoRender::initialize() { return 0; }

void NodeVideoRender::release() {}

int NodeVideoRender::deliverFrame(const IVideoFrame &videoFrame, int rotation,
                                  bool mirrored) {
  if (m_channel.get())
