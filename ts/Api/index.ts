...
class AgoraRtcEngine extends EventEmitter {
  ...
  initEventHandler(): void {
    ...
    this.rtcEngine.onEvent('userjoined', (
      ...
      console.log('ts user : ' + uid + ' joined.');
      let userAccount = this.getUserAccount(uid)
      console.log('ts duser : ' + userAccount + ' joined.');
    ...
    this.rtcEngine.onEvent('streamInjectStatus', (
      ...
      let userAccount = this.getUserAccount(uid)
      fire('streamInjectStatus', url, userAccount, status);
    });
  ...
  setupRemoteVideo(userAccount: string, view?: Element, channel?: string): number {
    let uid = this.getUid(userAccount);
    if(view) {
      //bind
  ...
  */
  ...
  registerVideoEncodedImageReceiver(callback: Function): number {
    return this.rtcEngine.registerVideoEncodedImageReceiver(callback);
  }

  unRegisterVideoEncodedImageReceiver(): number {
    return this.rtcEngine.unRegisterVideoEncodedImageReceiver();
  }
...
declare interface AgoraRtcEngine {
  ...
  on(
    evt: 'joinedChannel',
    cb: (channel: string, userAccount: string, elapsed: number) => void
  ): this;
   ...
   */
  on(evt: 'activeSpeaker', cb: (userAccount: string) => void): this;
   ...
   * - uid: The User ID.
   */
  on(evt: 'videoSourceJoinedSuccess', cb: (userAccount: string) => void): this;
  /** Occurs when the token expires. */
  ...
  on(
    ...
    cb: (url: string, userAccount: string, status: number) => void
  ): this;
...
class AgoraRtcChannel extends EventEmitter
{
  ...
  initEventHandler(): void {
    ...
    this.rtcChannel.onEvent('remoteVideoStats', (
      ...
      let statsCopy = Object.assign({}, {
        uid: this.rtcEngine.getUserAccount(stats.uid),
        delay: stats.delay,
        width: stats.width,
        height: stats.height,
        receivedBitrate: stats.receivedBitrate,
