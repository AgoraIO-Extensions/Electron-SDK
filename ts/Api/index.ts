...
class AgoraRtcEngine extends EventEmitter {
   ...
   * 
   * After the user successfully switches to another channel, the leavechannel and joinedChannel
   * callbacks are triggered to indicate that the user has left the original channel and joined a new one.
   * @param token The token generated at your server:
   * - For low-security requirements: You can use the temporary token generated at Dashboard. For details, see [Get a temporary token](https://docs.agora.io/en/Voice/token?platform=All%20Platforms#get-a-temporary-token).
   * - For high-security requirements: Set it as the token generated at your server. For details, see [Get a token](https://docs.agora.io/en/Voice/token?platform=All%20Platforms#get-a-token).
   * @param channel (Required) Pointer to the unique channel name for the Agora RTC session in the string format smaller than 64 bytes. Supported characters:
   * - The 26 lowercase English letters: a to z.
   * - The 26 uppercase English letters: A to Z.
   * - The 10 numbers: 0 to 9.
   * - The space.
   ...
   * and these callbacks return the state and events of the media stream 
   * relay.
   * - If the channelMediaRelayState callback returns 
   * TODO:RELAY_STATE_RUNNING(1) and RELAY_OK(0), and the channelMediaRelayEvent 
   * callback returns TODO:RELAY_EVENT_PACKET_SENT_TO_DEST_CHANNEL(4), the SDK 
   * starts relaying media streams between the original and the 
   * destination channel.
   * - If the channelMediaRelayState callback returns 
   * TODO:RELAY_STATE_FAILURE(3), an exception occurs during the media stream relay.
   * 
   * **Note**: 
   ...
   * the current relay.
   * @param config The configuration of the media stream relay: TODO:channelMediaRelayConfiguration.
   * @returns {number}
   * - 0: Success.
   * - < 0: Failure.
   */
   ...
   * 
   * After a successful method call, the SDK triggers the 
   * channelMediaRelayState callback with the 
   * TODO:RELAY_EVENT_PACKET_UPDATE_DEST_CHANNEL(7) state code.
   * @param config The media stream relay configuration: TODO:channelMediaRelayConfiguration.
   * @returns {number}
   * - 0: Success.
   * - < 0: Failure.
   */
   ...
   * 
   * After a successful method call, the SDK triggers the 
   * channelMediaRelayState callback. If the callback returns 
   * TODO:RELAY_STATE_IDLE(0) and RELAY_OK(0), the broadcaster successfully 
   * stops the relay.
   * 
   * **Note**:
   * If the method call fails, the SDK triggers the 
   * channelMediaRelayState callback with the 
   * TODO:RELAY_ERROR_SERVER_NO_RESPONSE(2) or 
   * RELAY_ERROR_SERVER_CONNECTION_LOST(8) state code. You can leave the 
   * channel by calling the {@link leaveChannel} method, and the media 
   * stream relay automatically stops.
   * @returns {number}
   * - 0: Success.
   * - < 0: Failure.
   */
...
declare interface AgoraRtcEngine {
   ...
   * 
   * **Note**:
   * When the state is TODO:LOCAL_AUDIO_STREAM_STATE_FAILED (3), see the 
   * `error` parameter for details.
   * - state State of the local audio:TODO:
   * - error The error information of the local audio:TODO:
   */
   ...
   * - state State of the remote audio:TODO:
   * - reason The reason of the remote audio state change:TODO:
   ...
   * - ChannelMediaRelayState TODO:
   * - ChannelMediaRelayError TODO:
   */
