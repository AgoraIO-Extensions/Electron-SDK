 ...
 */
  ...
  /*
   * 远端用户在加入频道后发生视频卡顿的累计时长（ms）。通话过程中，视频帧率设置不低于 5 fps 时，连续渲染的两帧视频之间间隔超过 500
   * ms，则记为一次视频卡顿。
   */
 ...
 */
  ...
  videoBitrate?: number
  /*
   * 外部音频流输入的采样率。默认值为 48000 Hz。详见 AudioSampleRateType 。
   * 声网建议目前采用默认值，不要自行设置。 
   */
...
/*
 * 该参数仅适用于 Android 和 iOS 平台。 摄像头方向设置。
 */
...
/*
 * IRtcEngineEventHandler 接口类用于 SDK 向 app 发送事件通知，app 通过继承该接口类的方法获取 SDK
 * 的事件通知。
 */
 ...
 */
   ...
   *  在本地用户的回调中，只要本地用户在发流，speakerNumber 始终为
   *  1。
  ...
  /*
   * 离开频道回调。
   * App 调用 leaveChannel 方法时，SDK 提示 app 离开频道成功。在该回调方法中，app 可以得到此次通话的总通话时长、SDK
   * 收发数据的流量等信息。
   *
   * @param connection Connection 信息。详见 RtcConnection 。
   *
   ...
   *
   * @param deviceType 设备类型定义。详见 MediaDeviceType 。
   *
   * @param deviceState 设备状态定义。 macOS 平台： 0: 设备就绪。
   *  8: 设备被拔出。 Windows 平台：详见 MediaDeviceStateType 。
   */
   ...
   * @param connection  Connection 信息。详见 RtcConnection 。 
   *
   * @param uid 图像尺寸和旋转信息发生变化的用户的用户 ID（本地用户的
   *  uid
   *  为 0）。
   *
   ...
   *
   * @param connection Connection 信息。详见 
   *  RtcConnection 
   *  。
   *
   ...
   *
   * @param reason 远端视频流状态改变的具体原因，详见 
   *  RemoteVideoStateReason 
   *  。 
   *
   * @param elapsed 从本地用户调用
   ...
   * 直播场景下，该回调提示有主播加入了频道。如果加入之前，已经有主播在频道中了，新加入的用户也会收到已有主播加入频道的回调。Agora 建议连麦主播不超过 17 人。 该回调在如下情况下会被触发： 远端用户/主播调用
   * joinChannelWithOptions
   * 方法加入频道。 远端用户加入频道后将用户角色改变为主播。
   * 远端用户/主播网络中断后重新加入频道。
   *
   * @param connection Connection 信息。详见 
   *  RtcConnection 
   ...
   * 网络连接中断回调。
   * 弃用:
   * 请改用 onConnectionStateChanged 回调。 SDK 在和服务器建立连接后，失去了网络连接超过 4 秒，会触发该回调。在触发事件后，SDK 会主动重连服务器，所以该事件可以用于 UI 提示。该回调与 onConnectionLost 的区别是： 
   * onConnectionInterrupted 回调一定是发生在成功加入频道后，且 SDK
   * 刚失去和服务器连接超过 4 秒时触发。
   * onConnectionLost 回调是无论是否成功加入频道，只要 10
   * 秒内和服务器无法建立连接都会触发。
  ...
  /*
   *  Token 已过期回调。 
   * 在通话过程中如果 Token 已失效，SDK 会触发该回调，提醒 app 更新 Token。 当收到该回调时，你需要重新在服务端生成新的 Token，然后调用 
   * joinChannelWithOptions 
   * 重新加入频道。
   *
   * @param connection Connection 信息。详见 RtcConnection 。
   ...
   *  ClientRoleBroadcaster (1)：主播。
   *  ClientRoleAudience (2)：观众。 
   *  ClientRoleType 。
   *
  ...
  /*
   *  旁路推流状态发生改变回调。 
   * 旁路推流状态发生改变时，SDK会触发该回调，并在回调中明确状态发生改变的 URL 地址及当前推流状态。
   ...
   *  ERR_FAILED (1): 推流失败。
   *  ERR_INVALID_ARGUMENT (2): 参数错误。如果你在调用 addPublishStreamUrl 前没有调用 setLiveTranscoding 配置 LiveTranscoding ，SDK 会回调
   ...
   *  相关错误。请调用 removePublishStreamUrl 
   *  方法删除原来的推流地址，然后调用 addPublishStreamUrl 
   *  方法重新推流到新地址。
   *  ERR_PUBLISH_STREAM_NUM_REACH_LIMIT (152):
   *  单个主播的推流地址数目达到上限 10。请删掉一些不用的推流地址再增加推流地址。
   *  ERR_PUBLISH_STREAM_NOT_AUTHORIZED (153):
   *  操作不属于主播自己的流，如更新其他主播的流参数、停止其他主播的流。请检查应用业务逻辑。
   *  ERR_PUBLISH_STREAM_INTERNAL_SERVER_ERROR (154):
   *  推流服务器出现错误。请调用 removePublishStreamUrl 重新推流。
   ...
   * 更新时，onTranscodingUpdated 回调会被触发并向主播报告更新信息。
   * 首次调用 setLiveTranscoding 方法设置转码参数
   * LiveTranscoding 时，不会触发此回调。
   */
   ...
   * @param uid 远端用户 ID。
   *
   * @param info 标识用户信息的 UserInfo 对象，包含用户 UID 和 User Account。详见 
   *  UserInfo 
   *  类。
   */
   ...
   * @param channel 频道名。
   *
   * @param oldState 之前的发布状态，详见 
   *  StreamPublishState 
   *  。
   *
   * @param newState 当前的发布状态，详见
   *  StreamPublishState
   *  。
   *
   * @param elapseSinceLastState 两次状态变化时间间隔（毫秒）。
 ...
 * 插拔设备不会改变设备 ID。
 *
 * @param deviceId 设备 ID。可通过调用 enumerateVideoDevices 方法获取。
 ...
 * 不同的频道场景下，SDK 的默认视频编码码率是不同的，详见 setVideoEncoderConfiguration 中的说明。
 *
 ...
 * 调用 enableVideo ，启用视频模块。
 * 调用该方法，开启虚拟背景功能。 
 * 该功能对设备性能要求较高，Agora 推荐你在搭载如下芯片的设备上使用： CPU 为 i5 及更好的设备 Agora 建议你在满足如下条件的场景中使用该功能： 使用高清摄像设备、摄像环境光照均匀。
 ...
 *
 * @param filePath 文件路径： Windows: 音频文件的绝对路径或 URL 地址，需精确到文件名及后缀。例如 C:\music\audio.mp4。
 ...
 * setVoiceConversionPreset
 *
 * @param preset SDK 预设的音效，支持以下设置： 
 *  RoomAcoustics3DVoice，3D 人声音效。 你需要在使用该枚举前将 setAudioProfile 的
 *  profile 参数设置 为 AudioProfileMusicStandardStereo(3) 或
 *  AudioProfileMusicHighQualityStereo(5)，否则该枚举设置无效。
 *  启用 3D 人声后，用户需要使用支持双声道的音频播放设备才能听到预期效果。 PitchCorrection，电音音效。为获取更好的人声效果，Agora
 *  建议你在使用该枚举前将 setAudioProfile 的
 *  profile 参数设置为 AudioProfileMusicHighQuality(4) 或 AudioProfileMusicHighQualityStereo(5)。
 *
 ...
 * 你需要在调用 setExternalAudioSink 之后、加入频道之前调用该方法。
 *
 * @param enabled 设置是否开启自定义回声消除： true：开启。
 *  false：关闭。
 *
 * @param audioSourceDelay 设置推送音频帧到发布音频帧中间的时间（毫秒）。取值范围为 [0,30]。 如需调用 pushCaptureAudioFrame1 ，请确保该参数设为 0。
 *  如需调用 pushReverseAudioFrame1 或处理声卡采集的音频帧，请确保该参数为 10 的整数倍。
 */
 ...
 * 设置采集的原始音频数据格式。
 * 该方法设置 回调的采集音频格式。
 * 调用 注册插件并获取采集的原始音频数据后，你可以调用本方法设置 SDK 返回的该原始数据的采样率、声道数等参数。 该方法需要在加入频道前调用。
 * SDK 会通过该方法中的 samplesPerCall、sampleRate 和 channel 参数计算出采样间隔，计算公式为采样间隔 = samplesPerCall/(sampleRate × channel)。请确保采样间隔不小于 0.01 秒。
 *
 * @param sampleRate SDK 返回数据的采样率 (Hz)，可设置为 8000、 16000、 32000、 44100 或 48000。
 *
 * @param channel SDK 返回数据的通道数。取值可为 1 或 2： 1: 单声道。
 *  2: 双声道。 
 *
 * @param mode SDK 返回数据的使用模式，详见 RawAudioFrameOpModeType 。
 *
 * @param samplesPerCall SDK 返回数据的采样点数，如旁路推流应用中通常为 1024。
 */
...
/*
 * 设置播放的音频格式。
 * 该方法设置 回调数据的格式。 该方法需要在加入频道前调用。
 * SDK 会通过该方法中的 samplesPerCall、sampleRate 和 channel 参数计算出采样间隔，计算公式为采样间隔 = samplesPerCall/(sampleRate × channel)。请确保采样间隔不小于 0.01 秒。SDK 会根据该采样间隔触发 回调。
 *
 * @param sampleRate  中返回数据的采样率，可设置为 8000、 16000、 32000、44100 或 48000。
 *
 * @param channel 中返回数据的通道数，可设置为 1 或 2: 
 *  1: 单声道
 *  2: 双声道 
 *
 * @param mode 
 *
 * @param samplesPerCall  中返回数据的采样点数，如旁路推流应用中通常为 1024。
 */
 ...
 * 设置 报告的音频数据格式。
 *
 * @param sampleRate 音频数据采样率 (Hz)，可设置为 8000、 16000、32000、 44100 或 48000。
 *
 * @param channel 音频数据声道数，可设置为 1（单声道） 或 2（双声道）。
 *
 * @param samplesPerCall 音频数据采样点数。旁路推流场景下通常设为 1024。
 */
 ...
 * 设置 报告的音频数据格式。
 *
 * @param null 音频数据声道数，可设置为 1（单声道） 或 2（双声道） 。
 *
 * @returns
 * 0: 方法调用成功
 * < 0: 方法调用失败
 ...
 *
 * @param null SDK 触发 onLocalAudioSpectrum 和 onRemoteAudioSpectrum 回调的时间间隔（毫秒）。 默认值为 100 毫秒。取值不得少于 10 毫秒，否则该方法会调用失败。
 *
 * @returns
 * 0: 方法调用成功。
 * < 0: 方法调用失败。 -2(ERR_INVALID_ARGUMENT): 参数设置错误
 */
...
/*
 * 开启声卡采集。
 * 启用声卡采集功能后，声卡播放的声音会被合到本地音频流中，从而可以发送到远端。 该方法仅适用于 macOS 和 Windows 平台。
 * macOS 系统默认声卡不支持采集功能，如果你需要使用该功能，请启用一个虚拟声卡，并将该虚拟声卡的名字传入 deviceName 参数。Agora 推荐你使用虚拟声卡 Soundflower 进行声卡采集。
 * 该方法在加入频道前后都能调用。
 *
 * @param enabled 是否开启声卡采集: 
 *  true: 开启声卡采集。
 *  false:（默认）关闭声卡采集。
 *
 * @param deviceName macOS: 虚拟声卡的设备名。默认为空，代表使用 Soundflower 虚拟声卡进行采集。
 ...
 * 该方法仅适用于 Windows。
 *
 * @param thumbSize 屏幕或窗口的缩略图的目标尺寸（宽高单位为像素）。详见 SIZE 。 SDK 会在保证原图不变形的前提下，缩放原图，使图片最长边和目标尺寸的最长边的长度一致。例如，原图宽高为 400 × 300，thumbSize 为 100 x 100，缩略图实际尺寸为 100 × 75。如果目标尺寸大于原图尺寸，缩略图即为原图，SDK 不进行缩放操作。
 *
 * @param iconSize 程序所对应的图标的目标尺寸（宽高单位为像素）。详见 SIZE。 SDK 会在保证原图不变形的前提下，缩放原图，使图片最长边和目标尺寸的最长边的长度一致。例如，原图宽高为 400 × 300，iconSize 为 100 × 100，图标实际尺寸为 100 × 75。如果目标尺寸大于原图尺寸，图标即为原图，SDK 不进行缩放操作。
 *
 * @param includeScreen 除了窗口信息外，SDK 是否返回屏幕信息： true：SDK 返回屏幕和窗口信息。 
 *  false：SDK 仅返回窗口信息。
 *
 * @returns
 * ScreenCaptureSourceInfo 数组。
 */
...
/*
 * 通过屏幕 ID 共享屏幕。
 * 开启屏幕共享有如下两种方案，你可以根据实际场景进行选择： 在加入频道前调用该方法，然后调用 joinChannelWithOptions 加入频道并设置 publishScreenTrack true，即可开始屏幕共享。
 * 在加入频道后调用该方法，然后调用 updateChannelMediaOptions 设置 publishScreenTrack true，即可开始屏幕共享。 共享一个屏幕或该屏幕的部分区域。用户需要在该方法中指定想要共享的屏幕 ID。
 *
 * @param displayId 指定待共享的屏幕 ID。开发者需要通过该参数指定你要共享的那个屏幕。 
 *
 * @param regionRect  Rectangle 。如果设置的共享区域超出了屏幕的边界，则只共享屏幕视窗内的内容；如果宽或高为 0，则共享整个屏幕。
 *
 ...
 * 通过指定区域共享屏幕。
 * 开启屏幕共享有如下两种方案，你可以根据实际场景进行选择： 在加入频道前调用该方法，然后调用 joinChannelWithOptions 加入频道并设置 publishScreenTrack true，即可开始屏幕共享。
 * 在加入频道后调用该方法，然后调用 updateChannelMediaOptions 设置 publishScreenTrack true，即可开始屏幕共享。 共享一个屏幕或该屏幕的部分区域。你需要在该方法中指定想要共享的屏幕区域。
 * 该方法仅适用于 Windows 平台。
 *
 * @param screenRect 指定待共享的屏幕相对于虚拟屏的位置。
 *
 * @param regionRect  Rectangle 。如果设置的共享区域超出了屏幕的边界，则只共享屏幕内的内容；如果将 width 或 height 设为 0 ，则共享整个屏幕。
 *
 ...
 * 通过窗口 ID 共享窗口。
 * 开启屏幕共享有如下两种方案，你可以根据实际场景进行选择： 在加入频道前调用该方法，然后调用 joinChannelWithOptions 加入频道并设置 publishScreenTrack true，即可开始屏幕共享。
 * 在加入频道后调用该方法，然后调用 updateChannelMediaOptions 设置 publishScreenTrack true，即可开始屏幕共享。 共享一个窗口或该窗口的部分区域。用户需要在该方法中指定想要共享的窗口 ID。
 * 该方法仅适用于 macOS 和 Windows 平台。
 * 该方法支持共享通用 Windows 平台（UWP）应用窗口。声网使用最新版 SDK 对主流的 UWP 应用进行了测试，结果如下：
 *
 * @param windowId 指定待共享的窗口 ID。
 *
 * @param regionRect （可选）指定待共享区域相对于整个屏幕的位置。如不填，则表示共享整个屏幕。详见 Rectangle 。如果设置的共享区域超出了窗口的边界，则只共享窗口内的内容；如果宽或高为 0，则共享整个窗口。
 *
 * @param captureParams 屏幕共享的参数配置。默认的分辨率为 1920 x 1080，即 2073600 像素。该像素值为计费标准。详见 ScreenCaptureParameters 。
 *
 * @returns
...
/*
 * 设置屏幕共享内容类型。
 * Agora SDK 会根据不同的内容类型，使用不同的算法对共享效果进行优化。如果不调用该方法，SDK 会将屏幕共享的内容默认为 ContentHintNone，即无指定的内容类型。
 * 该方法在开始屏幕共享前后都能调用。
 *
 ...
 * 更新屏幕共享区域。
 *
 * @param regionRect 待共享区域相对于整个屏幕或窗口的位置，如不填，则表示共享整个屏幕或窗口。详见 Rectangle 。
 *  如果设置的共享区域超出了屏幕或窗口的边界，则只共享屏幕或窗口内的内容；如果将 width 或 height 设为 0 ，则共享整个屏幕或窗口。
 *
 * @returns
 * 0: 方法调用成功。
 * < 0: 方法调用失败。 -3: 如果当前没有共享的屏幕，会返回该错误码。
 */
 ...
 * 开始通过第一个摄像头采集视频。
 *
 * @param config 视频采集配置。详见 CameraCapturerConfiguration 。
 *
 * @returns
 * 0: 方法调用成功
 * < 0: 方法调用失败
 ...
 * 开始共享第一个屏幕。
 *
 * @param config 屏幕采集配置。详见 ScreenCaptureConfiguration 。
 */
 ...
 * 启用内置加密，并设置数据加密密码。
 * 弃用：
 * 请改用 enableEncryption 方法。 在加入频道之前， app 需调用该方法指定 secret 来启用内置的加密功能，同一频道内的所有用户应设置相同的
 * secret。当用户离开频道时，该频道的 secret
 * 会自动清除。如果未指定 secret 或将 secret
 * 设置为空，则无法激活加密功能。 请不要在旁路推流时调用此方法。
...
/*
 * 输入在线媒体流。
 * 客户端输入在线媒体流功能即将停服。如果你尚未集成该功能，Agora 建议你不要使用。详见部分服务下架计划。 请确保已开通旁路推流的功能，详见旁路推流中的前提条件。
 * 在直播场景中，只有角色为主播的用户才能调用该方法。
 * 频道内同一时间只允许输入一个在线媒体流。
 * 该方法需要在加入频道后调用。 该方法将正在播放的音视频作为音视频源导入到正在进行的直播中。可主要应用于赛事直播、多人看视频互动等直播场景。调用该方法后，SDK 会在本地触发 回调，报告输入在线媒体流的状态；成功输入媒体流后，该音视频流会出现在频道中，频道内所有用户都会收到 onUserJoined 回调，其中 uid 为 666。
 *
 * @param url 添加到直播中的视频流 URL 地址。支持 RTMP、HLS、HTTP-FLV 协议传输。 支持的音频编码格式：AAC；
 *  支持的视频编码格式：H.264(AVC)。 
 *
 ...
 * @returns
 * 0: 方法调用成功
 * < 0: 方法调用失败 ERR_INVALID_ARGUMENT (2): 输入的 URL 为空。请重新调用该方法，并确认输入的媒体流的 URL 有效。
 ...
 * 如果你在旁路推流过程中调用了本方法离开频道，SDK 将自动调用 removePublishStreamUrl 
 * 方法。
 *
 * @returns
 * 0(ERR_OK): 方法调用成功。
 ...
 * 远端会触发 onUserJoined 或 onUserOffline (UserOfflineBecomeAudience) 回调。
 *
 * @param role 用户角色。详见 ClientRoleType 。
 *
 * @returns
 * 0: 方法调用成功。
 ...
 *  开始语音通话回路测试。 
 * 弃用：
 * 该方法已废弃，请改用 startEchoTest 。 该方法启动语音通话测试，目的是测试系统的音频设备（耳麦、扬声器等）和网络连接是否正常。在测试过程中，用户先说一段话，声音会在 10 秒后回放出来。如果 10 秒后用户能正常听到自己刚才说的话，就表示系统音频设备和网络连接都是正常的。 请在加入频道前调用该方法。 调用 startEchoTest [1/2] 后必须调用 stopEchoTest 以结束测试，否则不能进行下一次回声测试，也无法加入频道。 直播场景下，该方法仅能由用户角色为主播的用户调用。
 *
 * @returns
 * 0: 方法调用成功
 * < 0: 方法调用失败
...
/*
 * 开始播放音乐文件。
 * 该方法支持将本地或在线音乐文件和麦克风采集的音频进行混音或替换。成功播放音乐文件后，本地会触发 onAudioMixingStateChanged (AudioMixingStatePlaying) 回调。播放结束后，本地会触发 onAudioMixingStateChanged (AudioMixingStateStopped) 回调。 该方法在加入频道前后均可调用。如需多次调用 startAudioMixing [1/2]，请确保调用间隔大于 500 ms。
 ...
 * 设置本地视图显示模式。
 * 弃用：
 * 该方法已废弃，请使用 setLocalRenderMode 作为替代。 该方法设置本地视图显示模式。 App 可以多次调用此方法更改显示模式。
 *
 * @returns
 * 0: 方法调用成功。
 * < 0: 方法调用失败。
 ...
 * 双流指视频大流和视频小流： 视频大流：高分辨率、高帧率的视频流。
...
/*
 * 创建数据流。
 * 在 IRtcEngine 生命周期内，每个用户最多只能创建 5 个数据流。 该方法需要在加入频道后调用。
 * 不可将 reliable 设为 true 且将 ordered 设为 true。
 *
 * @param reliable 该数据流是否可靠： true: 接收方 5 秒内会收到发送方所发送的数据，否则会收到 onStreamMessageError 回调并获得相应报错信息。
 *  false: 接收方不保证收到，就算数据丢失也不会报错。 
 *
 * @param ordered 该数据流是否有序： true: 接收方会按照发送方发送的顺序收到数据包。
 ...
 * 弃用： 该方法已废弃，请使用 
 * addVideoWatermark 
 * 作为替代。 该方法将一张 PNG 图片作为水印添加到本地发布的直播视频流上，同一直播频道中的用户，旁路推流观众，甚至采集设备都能看到或采集到该水印图片。如果你仅仅希望在旁路直播推流中添加水印，请参考 
 * setLiveTranscoding 
 * 中描述的用法。 
 ...
 *
 * @param watermark 待添加在本地直播推流中的水印图片： 
 *  RtcImage 
 *  。
 *
 * @returns
 ...
 * 通信场景下的用户和直播场景下的主播加入频道后，远端会依次触发 onUserJoined 和 onUserInfoUpdated 回调。 用户成功加入频道后，默认订阅频道内所有其他用户的音频流和视频流，因此产生用量并影响计费。如果想取消订阅，可以通过调用相应的 mute 方法实现。
 * 为保证通信质量，请确保频道内使用同一类型的数据标识用户身份。即同一频道内需要统一使用 UID 或 User Account。如果有用户通过 Agora Web SDK 加入频道，请确保 Web 加入的用户也是同样类型。
 *
 * @param userAccount 用户 User Account。该参数用于标识实时音视频互动频道中的用户。你需要自行设置和管理用户的 User Account，并确保同一频道中每个用户的 User Account 是唯一的。 该参数为必填，最大不超过 255 字节，不可填 NULL。以下为支持的字符集范围（共 89 个字符）： 26 个小写英文字母 a-z
 *  26 个大写英文字母 A-Z
 ...
 * 销毁单个视频渲染器对象。
 *
 * @param view 需要销毁的 HTMLElement 对象。
 */
