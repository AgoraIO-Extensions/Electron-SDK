 ...
 * 请确保生成 Token 时传入的 app ID 和创建 IRtcEngine 实例时传入的 app ID
 * 一致。
 *
 ...
 * @param connection Connection 信息。详见 RtcConnection 。
 *
 * @param enabled 是否开启声卡采集： true: 开启声卡采集。
 *  false:（默认）不开启声卡采集。
 *
 ...
 * 水印坐标和 setVideoEncoderConfigurationEx 方法中的设置有依赖关系： 如果视频编码方向（ OrientationMode ）
 * 固定为横屏或自适应模式下的横屏，那么水印使用横屏坐标。
 * 如果视频编码方向（OrientationMode）
 * 固定为竖屏或自适应模式下的竖屏，那么水印使用竖屏坐标。
 * 设置水印坐标时，水印的图像区域不能超出 setVideoEncoderConfigurationEx
 ...
 * 创建数据流。
 * 弃用：
 * 该方法已废弃。请改用 createDataStreamEx 。 你可以调用该方法创建数据流并提高数据传输的可靠性和有序性。 请确保将 reliable 和 ordered 设为相同的值。
 * 在 IRtcEngine 生命周期内，每个用户最多只能创建 5 个数据流。
 * 频道内数据通道最多允许数据延迟 5 秒，若超过 5 秒接收方尚未收到数据流，则数据通道会向 app 报错。
 *
 * @param connection Connection 信息。详见 RtcConnection 。
 *
 * @param reliable 是否保证数据可靠性，即接收方是否需要在数据发送后的 5 秒内接收： true: 保证接收方会在数据发送后的 5 秒内接收。如果接收方在数据发送后的 5 秒内没有接收数据，SDK 会报错。
 *  false: 不保证收方会在数据发送后的 5 秒内接收。如果接收延迟或丢失数据，SDK 不会报错。 
 *
 * @param ordered 是否保证数据有序性，即接收方是否需要收到有序的数据： true：保证接收方会按照发送方发送的顺序收到数据包。
 *  false：不保证接收方按照发送方发送的顺序收到数据包。 
 *
 * @returns
 * 成功创建的数据流 ID：方法调用成功。
 * < 0: 创建数据流失败。
