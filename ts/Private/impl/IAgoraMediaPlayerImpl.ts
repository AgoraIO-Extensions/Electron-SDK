...
export class IMediaPlayerImpl implements IMediaPlayer {
  ...
  protected getApiTypeFromSwitchAgoraCDNSrc (src: string, syncPts: boolean = false): string {
  ...
  protected getApiTypeFromSwitchSrc (src: string, syncPts: boolean = true): string {
  ...
  setAudioPlaybackDelay (delayMs: number): number {
    const apiType = this.getApiTypeFromSetAudioPlaybackDelay(delayMs)
    const jsonParams = {
      'delay_ms': delayMs,
      toJSON: () => {
        return {
