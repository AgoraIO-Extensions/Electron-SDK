...
/*
 * 3: 远端音频流卡顿。在 RemoteAudioStateReasonNetworkCongestion
 * 的情况下，会报告该状态。
 */
...
/*
 * 2: 远端视频流正在解码，正常播放。在 RemoteVideoStateReasonNetworkRecovery、RemoteVideoStateReasonLocalUnmuted、RemoteVideoStateReasonRemoteUnmuted 的情况下，
 * 会报告该状态。
 */
...
/*
 * 4: 远端视频流播放失败。在 RemoteVideoStateReasonInternal
 * 的情况下，会报告该状态。
 */
 ...
 */
   ...
   * 1: 主播音频在输出音频的 FL 声道。如果主播上行音频是多声道，Agora
   * 服务器会先把多声道混音成单声道。
   * 2: 主播音频在输出音频的 FC 声道。如果主播上行音频是多声道，Agora
   * 服务器会先把多声道混音成单声道。
   * 3: 主播音频在输出音频的 FR 声道。如果主播上行音频是多声道，Agora
   * 服务器会先把多声道混音成单声道。
   * 4: 主播音频在输出音频的 BL 声道。如果主播上行音频是多声道，Agora
   * 服务器会先把多声道混音成单声道。
   * 5: 主播音频在输出音频的 BR 声道。如果主播上行音频是多声道，Agora
   * 服务器会先把多声道混音成单声道。
   * 0xFF 或取值大于 5: 该主播音频静音，Agora
   * 服务器移除该主播的音频。 取值不为 0 时，需要使用特殊的播放器。
   */
  audioChannel?: number
}
 ...
 */
  ...
  bitrate?: number
  /*
   * 是否采集鼠标用于屏幕共享： true:（默认）采集鼠标。
   * false: 不采集鼠标。 
   */
  captureMouseCursor?: boolean
  /*
   * 调用 startScreenCaptureByWindowId 方法共享窗口时，是否将该窗口前置： true: 前置窗口。
   * false:（默认）不前置窗口。 
   */
  windowFocus?: boolean
  /*
   * 待屏蔽窗口的 ID 列表。调用 startScreenCaptureByScreenRect 开启屏幕共享时，你可以通过该参数屏蔽指定的窗口。你可以在调用 updateScreenCaptureParameters 更新屏幕共享的配置参数时，通过该参数动态屏蔽指定的窗口。
   */
 ...
 */
   ...
   * 44100
   * 48000 
   * 如果把该参数设为 44100 或 48000，为保证录音效果，Agora 推荐录制 WAV 文件或
   * quality
   * 为
...
/*
 * 0: 初始状态。在成功调用 stopChannelMediaRelay 停止跨频道媒体流转发后， 
 * onChannelMediaRelayStateChanged 会回调该状态。
 */
RelayStateIdle = 0,
/*
 * 1: SDK 尝试跨频道。
...
RelayStateRunning = 2,
/*
 * 3: 发生异常，详见 onChannelMediaRelayStateChanged 的
 * code 参数提示的错误信息。
 */
RelayStateFailure = 3,
}
 ...
 * 本地用户调用 enableLocalAudio (false) 或 enableLocalVideo (false)
 * 关闭本地音频或视频采集。
 * 本地用户角色为观众。 
 */
