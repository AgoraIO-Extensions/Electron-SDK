...
const moveMacLib = async (fileNames = []) => {
  const allMoveTask = fileNames.map(fileName => {
    const filePromise = globPromise(
      path.join(
        __dirname,
        `../tmp/Agora_Native_SDK_for_Mac_*/libs/${fileName}.framework`
      )
    ).then(filePath => {
      return fs.move(
        filePath[0],
  ...
  return Promise.all(allMoveTask);
};
      ...
      .then(() => {
        ...
        if (os === "mac") {
          return globPromise(
            path.join(
              __dirname,
              "../tmp/Agora_Native_SDK_for_Mac_*/libs/AgoraRtcKit.framework/"
            )
          );
        } else {
          return globPromise(
