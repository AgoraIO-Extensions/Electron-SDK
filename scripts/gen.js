...
const generate = () => {
  ...
  if (platform === 'mac') {
    shell.echo(chalk.blue('Generating project file for mac, this will cost a little time...'));
    ...
    if (
      shell.exec('node-gyp configure --target=1.8.3 --dist-url=https://atom.io/download/electron --arch=ia32 --debug', { silent: true })
        .code !== 0
    ) {
      shell.echo(chalk.red('Failed to generate project file".'));
    } else {
