...
/**
 * AgoraVideoSourcetransporter provide IAgroaVideoSourceTransporter
 * implementation based on IPC.
 */
class AgoraVideoSourceTransporter : public IAgoraVideoSourceTransporter {
 ...
 private:
  void clear();
  int deliverFrame_I420(const agora::media::IVideoFrame &videoFrame,
                        int rotation, bool mirrored);
  void getVideoSize(agora::rtc::VIDEO_PROFILE_TYPE profile, int &width,
                    int &height);
  void copyAndCentreYuv(const unsigned char *srcYPlane,
                        ...
                        int srcStride, char *dstYPlane, char *dstUPlane,
                        char *dstVPlane, int dstStride);
...
const unsigned int AgoraVideoSourceTransporter::S_MAX_DATA_LEN =
    MAX_VIDEO_WIDTH * MAX_VIDEO_HEIGHT;

const unsigned int AgoraVideoSourceTransporter::S_BUF_LEN =
    (int) (MAX_VIDEO_WIDTH * MAX_VIDEO_HEIGHT * 1.5 + 50);

AgoraVideoSourceTransporter::AgoraVideoSourceTransporter(
    AgoraVideoSource &videoSource)
    : m_videoSource(videoSource) {}

AgoraVideoSourceTransporter::~AgoraVideoSourceTransporter() { clear(); }
...
int AgoraVideoSourceTransporter::deliverFrame(
...
int AgoraVideoSourceTransporter::deliverFrame_I420(
    const agora::media::IVideoFrame &videoFrame, int rotation, bool mirrored) {
  ...
  image_header_type *hdr =
      (image_header_type *) (pbuf + sizeof(image_frame_info));
  ...
  //     (uint8_t*)y, destWidth, (uint8_t*)u, destWidthU, (uint8_t*)v,
  //     destWidthU, destWidth, destHeight, kFilterNone);
  ...
  if (videoFrame.width() == width && videoFrame.height() == height) {
    ...
    I420Scale(planeY, stride0, planeU, strideU, planeV, strideV,
              videoFrame.width(), videoFrame.height(), (uint8_t *) y,
              dest_stride, (uint8_t *) u, width2, (uint8_t *) v, width2, width,
              height, kFilterNone);
  }
...
void AgoraVideoSourceTransporter::copyAndCentreYuv(
...
void AgoraVideoSourceTransporter::getVideoSize(VIDEO_PROFILE_TYPE profile,
